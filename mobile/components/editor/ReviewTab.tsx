...
export function ReviewTab({ eventId, state, onState }: { eventId: string; state: EventEditorState; onState: (v: EventEditorState) => void }) {
  ...
  function removeDraft() {
    Alert.alert(
      "Удалить черновик?",
      "Черновик будет удалён безвозвратно. Удаление разрешено только если мероприятие не публиковалось и в нём нет заказов или продаж.",
      [
  ...
  return <ScrollView contentContainerStyle={s.content}><Text style={s.title}>Проверка</Text><Text style={s.help}>Финальная сводка, публикация и жизненный цикл мероприятия.</Text>
    ...
    {canEdit && r.status === "DRAFT" && <View style={[s.card, s.deleteCard]}><Text style={s.cardTitle}>Удалить черновик</Text><Text style={s.help}>Если мероприятие не публиковалось и в нём нет заказов или продаж, его можно удалить полностью. Архивировать или восстанавливать его перед удалением не требуется.</Text><TouchableOpacity disabled={busy === "delete"} style={s.deleteButton} onPress={removeDraft}>{busy === "delete" ? <ActivityIndicator color="#B42318" /> : <Text style={s.deleteText}>Удалить черновик</Text>}</TouchableOpacity></View>}
    {canEdit && (r.archived || r.status !== "DRAFT") && <View style={[s.card, { borderColor: r.archived ? "#94A3B8" : "#F59E0B" }]}><Text style={s.cardTitle}>{r.archived ? "Мероприятие в архиве" : "Архивирование"}</Text><Text style={s.help}>{r.archived ? "Черновик можно восстановить для редактирования или удалить отдельной красной кнопкой выше." : "Архивирование скрывает мероприятие и закрывает публичные продажи, сохраняя историю."}</Text><TouchableOpacity disabled={busy === "archive"} style={[s.archive, r.archived && s.restore]} onPress={archive}>{busy === "archive" ? <ActivityIndicator /> : <Text style={s.archiveText}>{r.archived ? "Восстановить как черновик" : "Архивировать мероприятие"}</Text>}</TouchableOpacity></View>}
  </ScrollView>;
}
