...
export default function DashboardScreen() {
  ...
  const events = useMemo(() => {
    ...
    return (data?.events || []).filter((event) => event.status !== "PAST").filter((event) => !query || `${event.title} ${event.venue.name} ${event.venue.city}`.toLowerCase().includes(query)).sort((a, b) => {
