...
async function renderTicket(ticket: HtmlTicketInput, index: number) {
  ...
  const logoUrl = customLogo || atlasLogoDataUri(darkLogo);
  ...
  return `<section class="ticket-page${index ? " page-break" : ""}" dir="${locale === "he" ? "rtl" : "ltr"}" lang="${locale}" style="--ticket-bg:${design.backgroundColor};--ticket-accent:${design.accentColor};${background ? `--ticket-image:url('${esc(background)}')` : ""}">
    <div class="ticket-card">
      ...
      <img class="official-logo" src="${esc(logoUrl)}" alt="Atlas One">
      ${elements.join("")}
...
export async function generateTicketHtml(tickets: HtmlTicketInput[]) {
  ...
  return `<!doctype html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><style>
    ...
    *{box-sizing:border-box;-webkit-print-color-adjust:exact!important;print-color-adjust:exact!important}
    ...
    .official-logo{position:absolute;z-index:7;left:27px;top:24px;width:174px;height:50px;object-fit:contain;object-position:left center}
    ...
    .ticket-page[dir=rtl] .ticket-status{right:auto;left:24px}.ticket-page[dir=rtl] .official-logo{left:auto;right:27px;object-position:right center}
