...
export type OrderReviewAction = "approve" | "reject";
export type OrderReviewInput = { action: OrderReviewAction; note?: string };
...
async function captureProviderAuthorization(authorization: AuthorizationRow) {
  ...
  try {
    ...
    console.info("hyp.approval.capture_completed", {
      authorizationId: authorization.id,
      sourceTransId: authorization.hypTransId,
      captureTransId: result.captureTranId,
    });
  } catch (error) {
...
export async function reviewOrder(publicId: string, input: OrderReviewInput, actor: Actor) {
  const target = await db.order.findUnique({
    where: { publicId },
    select: { id: true, eventId: true, customerName: true, status: true, totalMinor: true },
  });
  ...
  if (input.action === "approve" && target.status === "PAID") {
    if (!authorization || authorization.provider !== "HYP" || authorization.hypCaptureTransId) {
    ...
    try {
      ...
      console.error("hyp.recovery.ticket_email_failed", { publicId, message: emailError });
    }
    await writeAudit(actor, {
      action: "REQUEST_CAPTURE_RECOVERED",
      entityType: "Order",
      entityId: target.id,
      summary: `Восстановлено реальное HYP-списание по заявке ${target.customerName}`,
      metadata: { publicId },
    });
    return { status: "PAID", emailSent, emailError, recoveredCapture: true, legacyDemoOrder: false };
  }
  ...
  if (input.action === "approve" && !legacyDemoOrder) {
    const active = await db.$queryRaw<Array<{ exists: number | bigint }>>`
      SELECT COUNT(*) AS exists FROM Reservation
      WHERE orderId = ${target.id} AND status = 'ACTIVE' AND expiresAt > CURRENT_TIMESTAMP
    `;
    if (Number(active[0]?.exists ?? 0) !== 1) {
      throw new Error("Срок резерва заявки истёк. Нельзя списывать оплату или выпускать билет");
    }
  }
  ...
  const order = await db.$transaction(async (tx) => {
    const current = await tx.order.findUnique({
      where: { publicId },
      include: {
        event: { select: { description: true } },
        items: { include: { table: true, seat: true } },
        tickets: true,
      },
    });
    ...
    const authRows = hasAuthorization
      ? await tx.$queryRaw<Array<{ provider: string; status: string; hypCaptureTransId: string | null }>>`
          SELECT provider, status, "hypCaptureTransId" FROM PaymentAuthorization WHERE orderId = ${current.id} LIMIT 1
        `
      : [];
    ...
    if (input.action === "reject") {
      ...
      return tx.order.update({
        ...
        data: {
          status: "REJECTED",
          reviewNote: input.note?.trim() || rejectionMessage,
          reviewedAt: new Date(),
          paymentDueAt: null,
        },
      });
    ...
    if (auth?.provider === "HYP" && (auth.status !== "CAPTURED" || !auth.hypCaptureTransId)) {
      throw new Error("HYP не подтвердил реальное списание оплаты");
    }
    ...
    const paid = await tx.order.update({
      ...
      data: {
        status: "PAID",
        reviewNote: input.note || (legacyDemoOrder ? "Одобрено как демо-заказ без авторизации оплаты" : null),
        reviewedAt: new Date(),
        paymentDueAt: null,
      },
    });
  ...
  await writeAudit(actor, {
    action: input.action === "approve"
      ? (legacyDemoOrder ? "LEGACY_REQUEST_APPROVED" : "REQUEST_APPROVED_AND_CAPTURED")
  ...
  return { status: order.status, emailSent, emailError, legacyDemoOrder, recoveredCapture: false };
}
