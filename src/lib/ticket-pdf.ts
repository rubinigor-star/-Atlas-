...
async function loadOfficialAtlasLogo(pdf:PDFDocument){if(!atlasLogoBytesPromise){const logoPath=path.join(process.cwd(),"public","branding","atlas-one-logo-official.jpg.b64");atlasLogoBytesPromise=readFile(logoPath,"utf8").then(encoded=>Buffer.from(encoded.trim(),"base64"))}const bytes=await atlasLogoBytesPromise;if(!bytes.length)throw new Error("Official Atlas One logo is empty");return pdf.embedJpg(bytes)}
