...
export async function createCategory(eventId: string, value: CategoryInput) {
  ...
  await db.ticketCategory.create({ data: {
    eventId, name: value.name, description, priceMinor: value.priceMinor, colorHex: value.colorHex,
    pricingMode: value.pricingMode, capacity: value.capacity, salesStart: salesStart || parent.salesStart,
    salesEnd: salesEnd || parent.salesEnd, maxPerOrder: value.maxPerOrder,
...
export async function updateCategory(eventId: string, value: CategoryInput & { categoryId: string }) {
  ...
  await db.$transaction([
    ...
    db.ticketCategory.update({ where: { id: value.categoryId }, data: {
      name: value.name, description, colorHex: value.colorHex, priceMinor: value.priceMinor, capacity: value.capacity,
      pricingMode: value.pricingMode, salesStart, salesEnd, maxPerOrder: value.maxPerOrder,
      priceTiers: value.pricingMode === "SCHEDULED" && earlyEnd && value.earlyBirdPriceMinor !== undefined ? { create: [
        { label: "Early Bird", priceMinor: value.earlyBirdPriceMinor, startsAt: salesStart, endsAt: earlyEnd },
  ...
  await db.$transaction(async (tx) => {
    ...
    for (const item of objects) {
      await tx.table.create({ data: {
        zoneId: zone.id, label: item.label, objectType: item.objectType, seats: item.seats, priceMode: item.priceMode,
        priceMinor: item.priceMinor, x: item.x, y: item.y, rotation: item.rotation, width: item.width, height: item.height,
        categoryId: item.categoryId,
        seatItems: { create: Array.from({ length: sellableTypes.has(item.objectType) ? item.seats : 0 }, (_, index) => ({
          label: `${item.label}-${index + 1}`, position: index + 1,
