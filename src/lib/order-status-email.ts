...
function shell(locale: Locale, title: string, body: string) { const settings = localeConfig[locale]; return `<!doctype html><html lang="${settings.tag}" dir="${settings.dir}"><body style="font-family:Arial,sans-serif;background:#f3f4f6;padding:24px"><div style="max-width:620px;margin:auto;background:#fff;border-radius:18px;padding:28px"><h1>${escapeHtml(title)}</h1>${body}</div></body></html>`; }
export async function sendApprovalRequestReceivedEmail(publicId: string, requestedLocale?: string) {
  ...
  if (order.status !== "PENDING_APPROVAL") throw new Error("ORDER_NOT_PENDING_APPROVAL");
  ...
  const body = `<p>${escapeHtml(order.customerName)},</p><h2>${escapeHtml(order.event.title)}</h2><p>${c.text}</p><p>${c.next}</p><p><strong dir="ltr">${escapeHtml(order.publicId)}</strong></p><p><a href="${orderUrl}">${c.open}</a></p>`;
