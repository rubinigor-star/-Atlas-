...
export async function sendApprovalRequestReceivedEmail(publicId: string, requestedLocale?: string) {
  ...
  const body = `<p>${escapeHtml(order.customerName)},</p><h2>${escapeHtml(order.event.title)}</h2><p>${c.text}</p><p>${c.next}</p><p><strong>${escapeHtml(order.publicId)}</strong></p><p><a href="${orderUrl}">Открыть заявку</a></p>`;
