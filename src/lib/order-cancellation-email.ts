...
export async function sendOrderCancellationEmail(publicId: string, refundedMinor: number) {
  ...
  const response = await fetch("https://api.resend.com/emails", {
    ...
    body: JSON.stringify({
      ...
      to: [recipient],
      subject: `Заказ отменён - ${order.event.title}`,
      html: `<div style="font-family:Arial,sans-serif;max-width:620px;margin:auto;color:#111827"><div style="background:#081426;color:white;padding:26px"><h1 style="margin:0">Заказ отменён</h1></div><div style="padding:26px"><p>Здравствуйте, ${escapeHtml(order.customerName)}.</p><p>Ваш заказ <strong>${escapeHtml(order.publicId)}</strong> на мероприятие <strong>${escapeHtml(order.event.title)}</strong> отменён.</p><p>Возврат на сумму <strong>${escapeHtml(amount)}</strong> успешно оформлен. Срок фактического зачисления средств на карту зависит от банка и платёжной системы.</p><p>Билеты по этому заказу больше недействительны.</p><p><a href="${orderUrl}">Открыть заказ</a></p></div></div>`,
    }),
  });
  ...
  if (!response.ok) {
    ...
    console.error("[cancellation-email]", { publicId, recipient, status: response.status, message });
