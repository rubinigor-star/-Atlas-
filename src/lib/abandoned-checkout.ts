...
export function ensureAbandonedCheckoutRuntime() {
  if (!initialized) initialized = (async () => {
    const statements = [
      ...
      `CREATE TABLE IF NOT EXISTS "RecoveryScenario" ("id" TEXT PRIMARY KEY,"organizationId" TEXT,"eventId" TEXT,"name" TEXT NOT NULL,"active" BOOLEAN NOT NULL DEFAULT TRUE,"abandonAfterMinutes" INTEGER NOT NULL DEFAULT 30,"createdAt" TIMESTAMP(3) NOT NULL DEFAULT CURRENT_TIMESTAMP,"updatedAt" TIMESTAMP(3) NOT NULL DEFAULT CURRENT_TIMESTAMP)`,
      `ALTER TABLE "RecoveryScenario" ADD COLUMN IF NOT EXISTS "abandonAfterMinutes" INTEGER NOT NULL DEFAULT 30`,
...
async function ensureDefaultScenario(organizationId: string) {
  ...
  const rows = await db.$queryRawUnsafe<Array<{ id: string }>>(`SELECT "id" FROM "RecoveryScenario" WHERE "organizationId"=$1 AND "eventId" IS NULL ORDER BY "createdAt" ASC LIMIT 1`, organizationId);
  if (rows[0]) return rows[0].id;
  const scenarioId = randomUUID();
  await db.$transaction(async tx => {
    await tx.$executeRawUnsafe(`INSERT INTO "RecoveryScenario" ("id","organizationId","name","abandonAfterMinutes") VALUES ($1,$2,$3,30)`, scenarioId, organizationId, "Стандартное восстановление покупки");
...
export async function optOutAbandonedCheckout(token: string) {
  await ensureAbandonedCheckoutRuntime();
  const rows = await db.$queryRawUnsafe<Array<{ id: string }>>(`UPDATE "AbandonedCheckout" SET "status"='OPTED_OUT',"optOutAt"=COALESCE("optOutAt",CURRENT_TIMESTAMP),"stopReason"='CUSTOMER_OPT_OUT',"updatedAt"=CURRENT_TIMESTAMP WHERE "token"=$1 AND "status" NOT IN ('RECOVERED','OPTED_OUT') RETURNING "id"`, token);
  if (rows[0]) await db.$executeRawUnsafe(`UPDATE "RecoveryAction" SET "status"='CANCELLED',"error"='CUSTOMER_OPT_OUT',"updatedAt"=CURRENT_TIMESTAMP WHERE "checkoutId"=$1 AND "status"='PENDING'`, rows[0].id);
