export const guestFieldKeys=["firstName","lastName","phone","email","gender","birthDate","city","facebook","instagram"] as const;
...
export const defaultGuestFields:GuestFieldConfig={
  ...
  email:{visible:true,required:true},gender:{visible:true,required:true},birthDate:{visible:true,required:true},city:{visible:true,required:true},
...
export function parseGuestFields(description:string):GuestFieldConfig{
  ...
  try{
    const parsed=JSON.parse(Buffer.from(found,"base64").toString("utf8"));
    return Object.fromEntries(guestFieldKeys.map(key=>{
      const fallback=defaultGuestFields[key];
      const saved=parsed?.[key];
      return [key,saved?{visible:saved.visible!==false,required:saved.required===true}:fallback];
    })) as GuestFieldConfig;
  }catch{return defaultGuestFields;}
}
