...
export const cloneEventSchema = z.object({
  ...
  if (doorsOpenAt > startsAt) context.addIssue({ code: "custom", path: ["doorsOpenAt"], message: "Открытие дверей не может быть позже начала мероприятия" });
  if (salesStart >= salesEnd) context.addIssue({ code: "custom", path: ["salesEnd"], message: "Окончание продаж должно быть позже начала продаж" });
  if (salesEnd > startsAt) context.addIssue({ code: "custom", path: ["salesEnd"], message: "Продажи должны завершиться не позже начала мероприятия" });
});

export type CloneEventInput = z.infer<typeof cloneEventSchema>;
export type CloneActor = { id: string; organizationId: string | null };
...
export async function cloneEvent(actor: CloneActor, input: CloneEventInput) {
  ...
  const result = await db.$transaction(async (tx) => {
    ...
    for (const category of source.categories) {
      const created = await tx.ticketCategory.create({
        data: {
          eventId: event.id, name: category.name, description: category.description, priceMinor: category.priceMinor,
          pricingMode: category.pricingMode, currency: category.currency, capacity: category.capacity, sold: 0,
          salesStart: shiftDate(category.salesStart, salesShift) ?? newSalesStart,
          salesEnd: shiftDate(category.salesEnd, salesShift) ?? new Date(input.salesEnd), minPerOrder: category.minPerOrder,
          maxPerOrder: category.maxPerOrder, hidden: category.hidden, colorHex: category.colorHex,
        },
      });
      categoryMap.set(category.id, created.id);
      for (const tier of category.priceTiers) {
        await tx.ticketPriceTier.create({ data: { categoryId: created.id, label: tier.label, priceMinor: tier.priceMinor, startsAt: shiftDate(tier.startsAt, salesShift)!, endsAt: shiftDate(tier.endsAt, salesShift)! } });
      }
    }
    ...
    for (const zone of source.zones) {
      ...
      for (const table of zone.tables) {
        const newTable = await tx.table.create({
          data: { zoneId: newZone.id, label: table.label, seats: table.seats, priceMinor: table.priceMinor, priceMode: table.priceMode, objectType: table.objectType, x: table.x, y: table.y, rotation: table.rotation, width: table.width, height: table.height, reserved: false, categoryId: table.categoryId ? categoryMap.get(table.categoryId) : null },
        });
        tableMap.set(table.id, newTable.id);
        for (const seat of table.seatItems) {
          await tx.seat.create({ data: { tableId: newTable.id, label: seat.label, position: seat.position, status: "AVAILABLE", categoryId: seat.categoryId ? categoryMap.get(seat.categoryId) : null } });
        }
      }
    }

    if (source.ticketTemplate) {
      await tx.ticketTemplate.create({ data: { eventId: event.id, name: source.ticketTemplate.name, canvasJson: source.ticketTemplate.canvasJson, backgroundColor: source.ticketTemplate.backgroundColor, accentColor: source.ticketTemplate.accentColor, textColor: source.ticketTemplate.textColor, logoUrl: source.ticketTemplate.logoUrl, backgroundUrl: source.ticketTemplate.backgroundUrl } });
    }
    if (input.copyPromoCodes) for (const promo of source.promoCodes) await tx.promoCode.create({ data: { eventId: event.id, code: promo.code, discountPercent: promo.discountPercent, active: promo.active } });
    ...
    for (const link of source.promoterLinks) {
      ...
      await tx.promoterLink.create({
        data: { eventId: event.id, promoterId, label: link.label, code: newCode(link.label), active: link.active, allocationType: link.allocationType, guestLimit: link.guestLimit, maxPerOrder: link.maxPerOrder, customPriceMinor: link.customPriceMinor, commissionBps: link.commissionBps, exclusive: link.exclusive, startsAt: shiftDate(link.startsAt, eventShift), endsAt: shiftDate(link.endsAt, eventShift), categoryId: link.categoryId ? categoryMap.get(link.categoryId) : null, tableId: link.tableId ? tableMap.get(link.tableId) : null },
      });
    }
