...
export async function sendOrderTicketEmail(publicId: string) {
  ...
  const ticketRows = order.tickets.map((ticket, index) => {
    ...
    const walletButton = walletReady
      ...
      : "";
    return `<tr><td style="padding:16px 0;border-top:${index === 0 ? "0" : "1px solid #e5e7eb"}"><table role="presentation" width="100%" cellspacing="0" cellpadding="0"><tr><td style="padding-right:16px"><div style="font-size:12px;color:#6b7280;text-transform:uppercase;letter-spacing:.08em">Билет ${index + 1}</div><div style="font-size:17px;font-weight:700;color:#111827;margin-top:4px">${escapeHtml(ticket.holderName)}</div><div style="font-size:14px;color:#4b5563;margin-top:3px">${escapeHtml(ticket.category.name)}</div></td><td align="right" style="vertical-align:middle">${walletButton}</td></tr></table></td></tr>`;
  }).join("");
  ...
  return sendResendEmail({
    ...
    html: `<!doctype html><html><body style="margin:0;background:#f3f4f6;padding:24px 10px;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Arial,sans-serif;color:#111827"><table role="presentation" width="100%" cellspacing="0" cellpadding="0"><tr><td align="center"><table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="max-width:640px;background:#ffffff;border-radius:20px;overflow:hidden;box-shadow:0 8px 30px rgba(17,24,39,.08)"><tr><td style="background:#081426;padding:30px 30px 28px;color:#ffffff"><div style="font-size:12px;font-weight:800;letter-spacing:.18em;color:#ff6555">ATLAS ONE</div><h1 style="margin:12px 0 6px;font-size:30px;line-height:36px">Ваши билеты готовы</h1><p style="margin:0;color:#cbd5e1;font-size:15px">Сохраните их в телефоне или используйте PDF во вложении.</p></td></tr><tr><td style="padding:28px 30px"><p style="margin:0 0 18px;font-size:16px">Здравствуйте, ${escapeHtml(order.customerName)}.</p><h2 style="margin:0 0 14px;font-size:23px">${escapeHtml(order.event.title)}</h2><table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="background:#f8fafc;border:1px solid #e5e7eb;border-radius:14px;padding:16px"><tr><td style="font-size:14px;line-height:22px;color:#374151"><strong>Дата:</strong> ${escapeHtml(formatDate(order.event.startsAt))}<br><strong>Место:</strong> ${escapeHtml(order.event.venue.name)}, ${escapeHtml(order.event.venue.address)}<br><strong>Заказ:</strong> ${escapeHtml(order.publicId)}</td></tr></table>${walletReady ? `<div style="margin-top:24px"><div style="font-size:18px;font-weight:800;margin-bottom:3px">Быстрый доступ на входе</div><div style="font-size:14px;color:#6b7280;margin-bottom:10px">Добавьте каждый билет отдельно в Apple Wallet. QR-код будет доступен даже без поиска письма.</div><table role="presentation" width="100%" cellspacing="0" cellpadding="0">${ticketRows}</table></div>` : ""}<div style="text-align:center;margin-top:26px"><a href="${orderUrl}" style="display:inline-block;background:#111827;color:#ffffff;text-decoration:none;padding:14px 22px;border-radius:11px;font-size:15px;font-weight:700">Открыть заказ и все билеты</a></div><p style="margin:22px 0 0;color:#6b7280;font-size:13px;line-height:19px">PDF со всеми билетами приложен к письму. Каждый QR-код уникален и действует только для одного прохода.</p></td></tr></table><div style="max-width:640px;color:#9ca3af;font-size:12px;line-height:18px;text-align:center;padding:18px 20px">Atlas One · Электронные билеты и быстрый вход</div></td></tr></table></body></html>`,
