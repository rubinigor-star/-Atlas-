...
async function sendResendEmail(input: {
  ...
  logType: "ticket-email" | "rejection-email";
}) {
...
async function getOrder(publicId: string) {
  const order = await db.order.findUnique({
    where: { publicId },
    include: { event: { include: { venue: true } }, tickets: { include: { category: true } } },
  });
...
export async function sendOrderRejectionEmail(publicId: string) {
  ...
  return sendResendEmail({
    publicId,
    recipient,
    subject: `Статус вашей заявки — ${order.event.title}`,
    html: `<div style="font-family:Arial,sans-serif;max-width:620px;margin:auto;color:#111827"><div style="background:#081426;color:white;padding:26px;border-radius:16px 16px 0 0"><div style="font-size:13px;letter-spacing:2px">ATLAS TICKETS</div><h1 style="margin:10px 0 0">Обновление по вашей заявке</h1></div><div style="padding:26px;border:1px solid #e5e7eb;border-top:0;border-radius:0 0 16px 16px"><p>Здравствуйте, ${escapeHtml(order.customerName)}.</p><h2>${escapeHtml(order.event.title)}</h2><p>К сожалению, ваша заявка не была подтверждена из-за высокой загрузки мероприятия.</p><p><strong>Ваша заявка сохранена в списке ожидания.</strong> Если появятся свободные места, организатор сможет связаться с вами дополнительно.</p><p>Авторизация оплаты отменена. Списание денежных средств не производилось.</p><p><strong>Дата:</strong> ${escapeHtml(formatDate(order.event.startsAt))}<br><strong>Место:</strong> ${escapeHtml(order.event.venue.name)}, ${escapeHtml(order.event.venue.address)}<br><strong>Номер заявки:</strong> ${escapeHtml(order.publicId)}</p><p style="text-align:center;margin-top:24px"><a href="${orderUrl}" style="display:inline-block;background:#111827;color:white;text-decoration:none;padding:13px 20px;border-radius:10px">Открыть заявку</a></p><p style="font-size:12px;color:#6b7280">Это автоматическое уведомление Atlas.</p></div></div>`,
    logType: "rejection-email",
  });
}
