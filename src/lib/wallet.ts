...
function setPixel(png:PNG,width:number,height:number,x:number,y:number,color:[number,number,number,number]){if(x<0||y<0||x>=width||y>=height)return;const i=(width*y+x)<<2;png.data[i]=color[0];png.data[i+1]=color[1];png.data[i+2]=color[2];png.data[i+3]=color[3]}
function atlasIcon(accentHex:string,size=174){const png=new PNG({width:size,height:size});const navy:[number,number,number,number]=[8,20,38,255];const white:[number,number,number,number]=[255,255,255,255];const [ar,ag,ab]=hexRgb(accentHex);const accent:[number,number,number,number]=[ar,ag,ab,255];const radius=Math.round(size*.23);for(let y=0;y<size;y++)for(let x=0;x<size;x++){const dx=Math.max(radius-x,0,x-(size-1-radius));const dy=Math.max(radius-y,0,y-(size-1-radius));if(dx*dx+dy*dy<=radius*radius)setPixel(png,size,size,x,y,navy)}const cx=size/2;const top=size*.22;const bottom=size*.76;for(let y=Math.floor(top);y<bottom;y++){const t=(y-top)/(bottom-top);const half=size*(.08+.19*t);const inner=Math.max(0,half-size*.075);for(let x=Math.floor(cx-half);x<=Math.ceil(cx+half);x++){if(Math.abs(x-cx)>=inner)setPixel(png,size,size,x,y,white)}}for(let y=Math.floor(size*.55);y<Math.floor(size*.63);y++)for(let x=Math.floor(size*.39);x<Math.floor(size*.61);x++)setPixel(png,size,size,x,y,white);const dotR=size*.075;const dotX=size*.77;const dotY=size*.23;for(let y=0;y<size;y++)for(let x=0;x<size;x++)if((x-dotX)**2+(y-dotY)**2<=dotR**2)setPixel(png,size,size,x,y,accent);return PNG.sync.write(png)}
function atlasStrip(backgroundHex:string,accentHex:string){const width=750,height=246;const png=new PNG({width,height});const bg=hexRgb(backgroundHex),ac=hexRgb(accentHex);for(let y=0;y<height;y++)for(let x=0;x<width;x++){const t=x/(width-1);const i=(width*y+x)<<2;png.data[i]=Math.round(bg[0]*(1-t)+ac[0]*t);png.data[i+1]=Math.round(bg[1]*(1-t)+ac[1]*t);png.data[i+2]=Math.round(bg[2]*(1-t)+ac[2]*t);png.data[i+3]=255}return PNG.sync.write(png)}
