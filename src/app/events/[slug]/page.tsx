...
const copy={
 ru:{buyFrom:"Купить билеты от",buy:"Купить билеты",about:"О мероприятии",faq:"Часто задаваемые вопросы",readMore:"Читать далее",readLess:"Свернуть",date:"Дата мероприятия",start:"Начало мероприятия",venue:"Площадка",language:"Язык мероприятия",secure:"Безопасная покупка",secureInfo:"Защищённая оплата и электронный билет"},
 he:{buyFrom:"רכישת כרטיסים החל מ־",buy:"רכישת כרטיסים",about:"אודות האירוע",faq:"שאלות נפוצות",readMore:"לקריאה נוספת",readLess:"צמצום",date:"תאריך האירוע",start:"תחילת האירוע",venue:"מקום האירוע",language:"שפת האירוע",secure:"רכישה מאובטחת",secureInfo:"תשלום מוגן וכרטיס דיגיטלי"},
 en:{buyFrom:"Get tickets from",buy:"Get tickets",about:"About the event",faq:"Frequently asked questions",readMore:"Read more",readLess:"Show less",date:"Event date",start:"Event start",venue:"Venue",language:"Event language",secure:"Secure checkout",secureInfo:"Protected payment and digital ticket"},
} as const;
...
export default async function EventPage({params,searchParams}:{params:Promise<{slug:string}>;searchParams:Promise<Record<string,string|undefined>>}){
 ...
 const[promoterLink,zones,languageSettings,commercialTerms]=await Promise.all([
  ...
  getEventLanguageSettings(event.id),getEffectiveEventTerms(event.id,event.organizationId),
 ]);
 ...
 const categories=event.categories.flatMap(category=>{if(category.hidden)return[];try{const standardPrice=effectiveTicketPrice(category,now);const channelPrice=validPromoterLink?.allocationType==="CATEGORY"&&validPromoterLink.categoryId===category.id&&validPromoterLink.customPriceMinor!==null?validPromoterLink.customPriceMinor:standardPrice;return[{...category,description:stripPricingMarketingStrategy(category.description),priceMinor:channelPrice,pricingPresentation:ticketPricePresentation(category,now),marketingStrategy:parsePricingMarketingStrategy(category.description)}]}catch{return[]}});
 const objects=zones.flatMap(zone=>zone.tables.map(table=>({...table,zone:{name:zone.name}}))); const media=parseEventMedia(event.description); const presentation=parseEventPresentation(event.description); const heroVideo=media.find(item=>item.type==="VIDEO")?.url; const links=media.filter(item=>item.type==="LINK"); const languageLabel=eventLanguageLabels[i18n.locale][languageSettings.primaryLanguage];
 ...
 return <main id="event-public-page" className={styles.page} style={pageStyle}>
  ...
  <section className={`${styles.hero} ${mobile.hero} ${desktopLayout.hero} ${palette.heroPalette}`}>
   ...
   <div className={`${metaAlignment.shell} ${desktopLayout.wideShell}`}><EventMetaStrip locale={i18n.locale} date={eventDate(event.startsAt,i18n.locale)} day={eventDay(event.startsAt,i18n.locale)} startTime={eventStartTime(event.startsAt,i18n.locale)} doorsOpenTime={presentation.doorsOpenTime} city={event.venue.city} venue={event.venue.name} address={event.venue.address} ageRestriction={presentation.ageRestriction}/></div>
  </section>
  <section className={`${styles.body} ${bodyLayout.body}`}><div className={`shell ${styles.bodyGrid} ${bodyLayout.shell} ${bodyLayout.grid} ${desktopLayout.wideShell} ${desktopLayout.bodyGrid}`}>
   <div className={bodyLayout.leftColumn}><EventMobileVideo title={event.title} videoUrl={heroVideo} posterUrl={event.posterUrl}/><article className={`${styles.contentCard} ${bodyLayout.content}`}>
    ...
    <div className={styles.detailsList}><div className={styles.detailItem}><CalendarDays size={21}/><div><strong>{local.date}</strong><span>{eventDay(event.startsAt,i18n.locale)}</span></div></div><div className={styles.detailItem}><Clock3 size={21}/><div><strong>{local.start}</strong><span>{eventStartTime(event.startsAt,i18n.locale)}</span></div></div><div className={styles.detailItem}><MapPin size={21}/><div><strong>{local.venue}</strong><span>{event.venue.name}<br/>{event.venue.address}</span></div></div><div className={styles.detailItem}><Languages size={21}/><div><strong>{local.language}</strong><span>{languageLabel}</span></div></div><div className={`${styles.detailItem} ${styles.detailWide}`}><ShieldCheck size={21}/><div><strong>{local.secure}</strong><span>{local.secureInfo}</span></div></div></div>
