...
export default async function EventSeatsPage({ params, searchParams }: { params:Promise<{slug:string}>; searchParams:Promise<Record<string,string|undefined>> }) {
  ...
  const [promoterLink,zones,commercialTerms,i18n,guestSeatLocks]=await Promise.all([
  ...
  const objects=zones.flatMap(zone=>zone.tables.map(table=>({
    id:table.id,label:table.label,seats:table.seats,priceMinor:table.priceMinor,priceMode:table.priceMode,objectType:table.objectType,x:table.x,y:table.y,rotation:table.rotation,width:table.width,height:table.height,reserved:table.reserved||table.seatItems.some(seat=>seat.status!=="AVAILABLE"&&table.priceMode==="WHOLE_TABLE"),categoryId:table.categoryId,
    seatItems:table.seatItems.map(seat=>({id:seat.id,label:seat.label,position:seat.position,status:guestSeatLocks.has(seat.id)&&guestSeatLocks.get(seat.id)!==validPromoterLink?.id?"BLOCKED" as const:seat.status,categoryId:seat.categoryId})),
  })));
  ...
  const assignmentCss=zones.flatMap(zone=>zone.tables.flatMap(table=>table.seatItems.flatMap(seat=>{
    if(seat.status!=="AVAILABLE"||guestSeatLocks.has(seat.id))return[];
