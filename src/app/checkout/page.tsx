...
import { CheckoutFormV2 } from "@/components/checkout-form-v2";
...
export default async function Checkout({searchParams}:{searchParams:Promise<Record<string,string|undefined>>}){
  ...
  const explicitCode=query.ref||query.channel;
  const rememberedCode=store.get("atlas_promoter_channel")?.value;
  const candidateCode=explicitCode||rememberedCode;
  ...
  const[event,categories,tables,seats,promoterChannel,eventOrderLimit]=await Promise.all([
