...
export default async function Checkout({searchParams}:{searchParams:Promise<Record<string,string|undefined>>}){
  ...
  const[event,categories,tables,seats,promoterLink]=await Promise.all([
  ...
  const labels:string[]=[];const categoryQuantities=new Map<string,{quantity:number;minPerOrder:number;maxPerOrder:number}>();let subtotal=0;let quantity=0;
  const addCategoryQuantity=(category:{id:string;minPerOrder:number;maxPerOrder:number},amount:number)=>{const current=categoryQuantities.get(category.id);categoryQuantities.set(category.id,{quantity:(current?.quantity??0)+amount,minPerOrder:category.minPerOrder,maxPerOrder:category.maxPerOrder});};
  for(const item of items){
    ...
    if(item.tableId){
      ...
      addCategoryQuantity(table.category,table.seats);
      ...
      for(const seat of itemSeats)addCategoryQuantity(seat!.category!,1);
      ...
      addCategoryQuantity(category,item.quantity);
      labels.push(`${category.name} × ${item.quantity}`);
    }
  }
  if([...categoryQuantities.values()].some(item=>item.quantity<item.minPerOrder||item.quantity>item.maxPerOrder))notFound();
