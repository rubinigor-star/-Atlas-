...
export default async function EventFinancePage({params}:{params:Promise<{eventId:string}>}){
  ...
  return <AdminShell>
    ...
    <section className="panel" style={{marginBottom:20}}><div className="row between" style={{alignItems:"stretch",gap:16,flexWrap:"wrap"}}>
      <div style={{flex:"1 1 260px",padding:16,border:"1px solid #dbe7dc",borderRadius:14,background:"#f6fff8"}}><span className="muted">Доступно к выплате</span><h2 style={{margin:"6px 0",color:"#15803d"}}>{money(event.availableMinor)}</h2><small>Сумма открывается только после наступления стандартного payout-cycle 7-го числа.</small></div>
      <div style={{flex:"1 1 260px",padding:16,border:"1px solid #f0dfb8",borderRadius:14,background:"#fffaf0"}}><span className="muted">Ожидает поступления</span><h2 style={{margin:"6px 0",color:"#a16207"}}>{money(Math.max(0,event.awaitingSettlementMinor))}</h2><small>Будет учтено после расчётного цикла платёжной системы 6-го числа.</small></div>
      <div style={{flex:"1 1 260px",padding:16,border:"1px solid #d9e4f8",borderRadius:14,background:"#f7faff"}}><span className="muted">Плановая выплата</span><h2 style={{margin:"6px 0",color:"#1d4ed8"}}>{dateLabel(event.payoutDate)}</h2><small>Следующий стандартный цикл после проведения мероприятия.</small></div>
    </div></section>
