...
export default async function OrganizerCompanyPage(){
  ...
  return <AdminShell>
    ...
    <OrganizerDocumentsForm organizationId={organizationId} bank={{provided:Boolean(compliance.bankDocumentPath),name:compliance.bankDocumentName,updatedAt:documentDate(compliance.bankAccountUpdatedAt)}} tax={{provided:Boolean(compliance.taxDocumentPath),name:compliance.taxDocumentName,updatedAt:documentDate(compliance.taxDocumentUpdatedAt)}}/>
    <section className="platform-section-card"><span className="eyebrow">Договор и выплаты</span><h2>Статус compliance</h2><div className="platform-readiness-grid">{readiness.checks.map(check=><div key={check.key} className={`platform-readiness-item ${check.ready?"ready":"missing"}`}><b>{check.ready?"✓":"!"}</b><div><strong>{check.label}</strong><small>{check.ready?"Готово":"Потребуется до выплаты"}</small></div></div>)}</div>
      ...
      {agreementHistory.length>0&&<details className="platform-contract-card"><summary style={{cursor:"pointer",fontWeight:800}}>История принятых договоров ({agreementHistory.length})</summary>{agreementHistory.map(item=><div key={item.id} style={{padding:"10px 0",borderTop:"1px solid #e5e7eb"}}><strong>{item.version}</strong><br/><small>{dateLabel(item.acceptedAt)} · {item.acceptedByName} · {item.acceptedByEmail}</small><details><summary>Показать текст версии</summary><pre>{item.text}</pre></details></div>)}</details>}
      {!readiness.ready&&<p className="muted">Актуальный договор, банковский документ и ניכוי מס במקור можно завершить после регистрации. Пока checklist не готов, Atlas не позволит финально зафиксировать выплату.</p>}
    </section>
  </AdminShell>;
}
