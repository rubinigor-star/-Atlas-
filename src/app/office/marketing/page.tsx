...
export default async function MarketingPage() {
  ...
  const [events,paidOrders,recentOrders,campaigns,settings,customerOrders,promoters,partnerLinks,promoCodes,consents,suppressions]=await Promise.all([
  ...
  for(const order of customerOrders){
    ...
    audience.set(key,{key,guestId:previous?.guestId??order.guestId,name:previous?.name??order.customerName,email:previous?.email??order.customerEmail,phone:previous?.phone??order.customerPhone,city:previous?.city??order.customerCity,orders:(previous?.orders??0)+1,totalMinor:(previous?.totalMinor??0)+order.totalMinor,lastPurchaseAt:previous&&previous.lastPurchaseAt>order.createdAt?previous.lastPurchaseAt:order.createdAt});
  }
  const consentMap=new Map<string,Partial<Record<Channel,"GRANTED"|"REVOKED"|"UNKNOWN">>>();
  for(const row of consents)consentMap.set(row.guestId,{...consentMap.get(row.guestId),[row.channel]:row.status});
  const suppressionMap=new Map<string,{channels:Channel[];all:boolean}>();
  for(const row of suppressions){const current=suppressionMap.get(row.guestId)??{channels:[],all:false};if(row.channel)current.channels.push(row.channel);else current.all=true;suppressionMap.set(row.guestId,current);}
  const customers=[...audience.values()].sort((a,b)=>b.lastPurchaseAt.getTime()-a.lastPurchaseAt.getTime()).map(customer=>({guestId:customer.guestId,name:customer.name,email:customer.email,phone:customer.phone,city:customer.city,orders:customer.orders,totalMinor:customer.totalMinor,consents:customer.guestId?consentMap.get(customer.guestId)??{}:{},suppressed:customer.guestId?suppressionMap.get(customer.guestId)?.channels??[]:[],fullySuppressed:customer.guestId?suppressionMap.get(customer.guestId)?.all??false:false}));
  ...
  return <AdminShell>
    <div className="row between"><div><span className="eyebrow">Atlas Marketing</span><h1>Рассылки, автоматизации и реклама</h1><p className="muted">Единый кабинет клиентской базы, согласий, Email, SMS, WhatsApp, рекламных ссылок и атрибуции продаж.</p></div><button className="btn" disabled title="Реальная отправка включится после настройки провайдеров и тарифов">+ Новая кампания</button></div>
