...
export default async function PromoterDetailPage({params,searchParams}:{params:Promise<{id:string}>;searchParams:Promise<Record<string,string|undefined>>}){
 ...
 const assignments=await promoterV2AssignmentAnalytics(promoter.id,from);const clicks=assignments.reduce((s,x)=>s+x.clicks,0),checkouts=assignments.reduce((s,x)=>s+x.checkouts,0),abandoned=assignments.reduce((s,x)=>s+x.abandoned,0),recovered=assignments.reduce((s,x)=>s+x.recovered,0),orders=assignments.reduce((s,x)=>s+x.orders,0),tickets=assignments.reduce((s,x)=>s+x.tickets,0),revenue=assignments.reduce((s,x)=>s+x.revenue,0),conversion=clicks?orders/clicks*100:0;const label=PERIODS.find(x=>x.value===period)?.label||"30 дней";
 return <AdminShell>
  ...
  {staff.permissionSet.has("EVENT_MANAGE")?<div style={{marginTop:24}}><PromoterV2Detail promoter={{id:promoter.id,name:promoter.name,email:promoter.email,phone:promoter.phone,active:promoter.active,autoAssignAllEvents:promoter.autoAssignAllEvents,defaultCommissionBps:promoter.defaultCommissionBps}} events={events.map(e=>({id:e.id,title:e.title,status:e.status}))} assignments={assignments}/></div>:<div className="panel" style={{marginTop:24}}><p className="muted">У вас есть доступ к аналитике, но нет прав на управление промоутером.</p></div>}
 </AdminShell>;
}
