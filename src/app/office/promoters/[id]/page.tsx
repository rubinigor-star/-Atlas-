...
export default async function PromoterDetailPage({params,searchParams}:{params:Promise<{id:string}>;searchParams:Promise<Record<string,string|undefined>>}){
  ...
  const codeToAbandons=new Map<string,AbandonRow[]>();
  for(const row of abandons){const code=refCode(row.metadataJson);if(!code)continue;const list=codeToAbandons.get(code)||[];list.push(row);codeToAbandons.set(code,list);}
  const linkRows=links.map(link=>{
    const rows=codeToAbandons.get(link.code.toUpperCase())||[];
    const paidOrders=new Map(link.orders.map(order=>[order.id,order]));
    const orders=paidOrders.size;
    const tickets=[...paidOrders.values()].reduce((sum,order)=>sum+order.items.reduce((x,item)=>x+item.quantity,0),0);
    const revenue=[...paidOrders.values()].reduce((sum,order)=>sum+order.totalMinor,0);
    const clicks=link.visits.length;
    const checkout=new Set(rows.map(row=>row.token)).size;
    const abandoned=rows.filter(row=>row.abandonedAt).length;
    const recovered=rows.filter(row=>row.status==="RECOVERED").length;
    return {link,clicks,checkout,abandoned,recovered,orders,tickets,revenue,conversion:clicks?orders/clicks*100:0};
  });
  const clicks=linkRows.reduce((s,x)=>s+x.clicks,0);const checkout=linkRows.reduce((s,x)=>s+x.checkout,0);const abandoned=linkRows.reduce((s,x)=>s+x.abandoned,0);const recovered=linkRows.reduce((s,x)=>s+x.recovered,0);const orders=linkRows.reduce((s,x)=>s+x.orders,0);const tickets=linkRows.reduce((s,x)=>s+x.tickets,0);const revenue=linkRows.reduce((s,x)=>s+x.revenue,0);const conversion=clicks?orders/clicks*100:0;
