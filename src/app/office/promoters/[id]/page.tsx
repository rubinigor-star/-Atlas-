...
export default async function PromoterDetailPage({params,searchParams}:{params:Promise<{id:string}>;searchParams:Promise<Record<string,string|undefined>>}){
 ...
 return <AdminShell>
  ...
  {staff.permissionSet.has("EVENT_MANAGE")&&<PromoterOperationalManager promoterId={promoter.id} email={promoter.email} autoAssignAllEvents={automation.get(promoter.id)??false} events={events.map(e=>({id:e.id,title:e.title}))} assignments={allPromoterLinks.map(link=>{const note=notifications.get(link.id);return{id:link.id,eventId:link.eventId,eventTitle:link.event.title,eventStatus:link.event.status,shareUrl:`${PUBLIC_ORIGIN}/events/${link.event.slug}?channel=${encodeURIComponent(link.code)}`,emailStatus:note?.status??"NOT_SENT",sentAt:note?.sentAt?.toISOString()??null,active:link.active}})}/>} 
