...
export default async function PlatformFinancePage(){
  ...
  return <PlatformShell>
    <div className="platform-heading"><div><span className="eyebrow">Atlas Finance Control</span><h1>Финансы платформы</h1><p>Внутренняя финансовая картина Atlas: комиссии, дополнительные сервисы, обязательства перед организаторами, возвраты и выплаты.</p></div><span className="platform-admin-badge">SUPER ADMIN</span></div>
    <div className="stats">
      ...
      <div className="stat"><span className="muted">Доход Atlas</span><strong>{money(atlasRevenue)}</strong><small>продажи + отмены + сервисы</small></div>
      ...
      <div className="stat"><span className="muted">Можно выплатить сейчас</span><strong>{money(summary.availableForPayoutMinor)}</strong><small>только наступившие payout-cycle</small></div>
    </div>

    <div className="platform-card" style={{marginBottom:20}}><div className="row between"><div><span className="eyebrow">Доход Atlas</span><h2 style={{margin:"6px 0"}}>Структура дохода</h2></div></div><div className="stats" style={{marginBottom:0}}><div className="stat"><span className="muted">Комиссии с продаж</span><strong>{money(summary.atlasSalesFeeMinor)}</strong></div><div className="stat"><span className="muted">Комиссии с отмен</span><strong>{money(summary.atlasCancellationFeeMinor)}</strong></div><div className="stat"><span className="muted">Доп. сервисы</span><strong>{money(summary.atlasServicesMinor)}</strong><small>сейчас: успешно отправленные SMS</small></div></div></div>

    <div className="row between"><div><h2 className="section-title">Организаторы</h2><p className="muted">То, что видит только Atlas. В кабинете организатора внутренние комиссии не раскрываются.</p></div></div>
    <div className="table-wrap"><table><thead><tr><th>Организатор</th><th>Мероприятий</th><th>Оплачено клиентами</th><th>Заработок организатора</th><th>Возвраты</th><th>Сервисы</th><th>Баланс</th><th>Доход Atlas</th><th>Доступно к выплате</th></tr></thead><tbody>
      {summary.organizations.map(org=><tr key={org.organizationId}><td><Link href={`/platform/organizers/${org.organizationId}`}><strong>{org.organizationName}</strong></Link></td><td>{org.eventCount}</td><td>{money(org.buyerPaidMinor)}</td><td>{money(org.salesMinor)}</td><td>{money(org.refundsMinor)}</td><td>{money(org.servicesMinor)}</td><td><strong>{money(org.balanceMinor)}</strong></td><td style={{fontWeight:700,color:"#15803d"}}>{money(org.atlasRevenueMinor)}</td><td>{money(org.availableMinor)}</td></tr>)}
