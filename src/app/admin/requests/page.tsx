...
const copy={
  ru:{eyebrow:"Проверка гостей",title:"Заявки на билеты",description:"Проверяйте клиента, меняйте статус прямо в списке и связывайтесь с ним через WhatsApp.",live:"Обновляется автоматически"},
  he:{eyebrow:"אישור אורחים",title:"בקשות לכרטיסים",description:"בדקו את הלקוח, עדכנו את הסטטוס ישירות ברשימה וצרו איתו קשר ב־WhatsApp.",live:"מתעדכן אוטומטית"},
  en:{eyebrow:"Guest approval",title:"Ticket requests",description:"Review customers, update status directly in the list, and contact them via WhatsApp.",live:"Updates automatically"}
} as const;
...
export default async function RequestsPage() {
  ...
  const requests = approvalOrderIds.length
    ? await db.order.findMany({where:{id:{in:approvalOrderIds},status:{in:["PENDING_APPROVAL","AWAITING_PAYMENT","PAID","REJECTED","CANCELLED"]},OR:[{reviewNote:null},{reviewNote:{not:DISMISSED_EXPIRED_NOTE}}]},include:{event:true,items:true,guest:{include:{orders:{include:{tickets:{include:{scans:true}}}}}}},orderBy:[{status:"asc"},{createdAt:"desc"}]})
    : [];
  const [recoveryRows,activeReviewIds]=await Promise.all([
    db.$queryRaw<Array<{ orderId: string }>>`SELECT pa."orderId" FROM "PaymentAuthorization" pa JOIN "Order" o ON o."id"=pa."orderId" WHERE o."salesFlow"='APPROVAL' AND pa.provider='HYP' AND pa.status='CAPTURED' AND pa."hypCaptureTransId" IS NULL`,
  ...
  return <AdminShell><div className="office-page-heading"><div><span className="eyebrow">{text.eyebrow}</span><h1>{text.title}</h1><p>{text.description}</p></div><span className="office-live"><i />{text.live}</span></div><RequestInbox initialRequests={visibleRequests.map((request)=>{
