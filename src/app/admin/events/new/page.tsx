...
export default async function NewEvent(){
  ...
  return <AdminShell><div className="row between"><div><span className="eyebrow">Events</span><h1>Новое мероприятие</h1><p className="muted">Создайте одиночное мероприятие или объедините несколько дат в один тур.</p></div><Link href="/office/tours/new" className="btn dark">+ Создать тур</Link></div><details className="panel" open><summary><strong>Создать одиночное мероприятие</strong></summary><CreateEventForm/></details><details className="panel" style={{marginTop:20}}><summary><strong>Скопировать существующее</strong></summary><CloneEventForm events={events.map(event=>({id:event.id,title:event.title,startsAt:event.startsAt.toISOString(),venueName:event.venue.name,city:event.venue.city,address:event.venue.address}))}/></details></AdminShell>;
}
