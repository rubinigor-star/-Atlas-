...
export default async function ManageEvent({params,searchParams}:{params:Promise<{id:string}>;searchParams?:Promise<{tab?:string}>}){
 ...
 const map=<div className="stack">{(canManage||canTickets)&&<AdmissionModeManager eventId={event.id} initialMapEnabled={event.mapEnabled}/>} {event.mapEnabled&&canTickets?<><section className="panel venue-map-context"><span className="eyebrow">Схема зала</span><h2>{event.venue.name}</h2><p className="muted">Карта площадки редактируется ниже. Назначение цветов происходит через типы билетов.</p></section><FullscreenMapPanel><VenueMapEditor eventId={event.id} categories={event.categories.map(category=>({id:category.id,name:category.name,priceMinor:category.priceMinor,colorHex:category.colorHex}))} initialObjects={initialMapObjects}/></FullscreenMapPanel></>:event.mapEnabled?<div className="panel">У вас нет доступа к редактированию карты.</div>:<section className="panel"><h2>Карта не используется</h2><p className="muted">Покупатель выбирает тип и количество билетов без выбора места.</p></section>}</div>;
