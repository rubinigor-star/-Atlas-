...
export default async function ManageEvent({params,searchParams}:{params:Promise<{id:string}>;searchParams?:Promise<{tab?:string}>}){
 ...
 const readingStarterPlaceholder=isReadingVenue(event.venue.name)&&storedMapObjects.length<=2&&storedMapObjects.every(item=>!item.reserved)&&storedMapObjects.some(item=>item.label.trim().toLowerCase()==="r1"||item.objectType==="ROW");
 const initialMapObjects=isReadingVenue(event.venue.name)&&(storedMapObjects.length===0||readingStarterPlaceholder)?readingVenuePreset():storedMapObjects;
 ...
 const map=<div className="stack">{(canManage||canTickets)&&<AdmissionModeManager eventId={event.id} initialMapEnabled={event.mapEnabled}/>} {event.mapEnabled&&canTickets?<><section className="panel venue-map-context"><span className="eyebrow">Схема зала</span><h2>{event.venue.name}</h2><p className="muted">Карта площадки редактируется ниже. Назначение цветов происходит через типы билетов.</p></section><FullscreenMapPanel><VenueMapEditor eventId={event.id} venueName={event.venue.name} categories={event.categories.map(category=>({id:category.id,name:category.name,priceMinor:category.priceMinor,colorHex:category.colorHex}))} initialObjects={initialMapObjects}/></FullscreenMapPanel></>:event.mapEnabled?<div className="panel">У вас нет доступа к редактированию карты.</div>:<section className="panel"><h2>Карта не используется</h2><p className="muted">Покупатель выбирает тип и количество билетов без выбора места.</p></section>}</div>;
 ...
 const review=<div className="stack"><section className="panel"><span className="eyebrow">Проверка</span><h2>{event.title}</h2><div className="stats"><div className="stat"><span className="muted">Статус</span><strong>{archived?"ARCHIVED":event.status}</strong></div><div className="stat"><span className="muted">Формат</span><strong>{event.mapEnabled?"С выбором мест":"Без схемы"}</strong></div><div className="stat"><span className="muted">Категорий билетов</span><strong>{event.categories.length}</strong></div></div><div className="row" style={{flexWrap:"wrap"}}><Link className="btn dark" href={`/office/events/${event.id}/preview`}>Предварительный просмотр</Link>{canManage&&<Link className="btn secondary" href={`/office/events/${event.id}/links`}>Продажи и гостевые ссылки</Link>}{event.status==="PUBLISHED"&&<Link className="btn secondary" href={`/events/${event.slug}`}>Открыть публичную страницу</Link>}</div></section>{canManage&&<EventPublishControl eventId={event.id} status={event.status as "DRAFT"|"PUBLISHED"}/>} {(canManage||canTickets)&&<EventAtlasAssistant event={{id:event.id,title:event.title,status:archived?"ARCHIVED":event.status,salesMode:event.salesMode,startsAt:event.startsAt.toISOString(),venue:`${event.venue.name}, ${event.venue.city}`,categories:event.categories.map(item=>({id:item.id,name:item.name,priceMinor:item.priceMinor,capacity:item.capacity,sold:item.sold,pricingMode:item.pricingMode}))}}/>}{canManage&&<EventArchiveControl eventId={event.id} eventTitle={event.title} status={event.status as "DRAFT"|"PUBLISHED"} archived={archived}/>}</div>;

 return <AdminShell><div className="row between"><div><span className="eyebrow">Единый редактор мероприятия</span><h1>{event.title}</h1><p className="muted">Каждая настройка находится только в одном разделе.</p></div><span className="pill">{event.status}</span></div><EventEditorWorkspace initialTab={initialTab} about={about} tickets={tickets} map={map} checkout={checkout} review={review}/></AdminShell>;
}
