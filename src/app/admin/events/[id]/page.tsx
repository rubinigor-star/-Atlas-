...
import { BuyerQuestionManager } from "@/components/buyer-question-manager";
...
export default async function ManageEvent({params}:{params:Promise<{id:string}>}){
 ...
 const media=parseEventMedia(event.description);const buyerQuestions=parseBuyerQuestions(event.description);const cleanDescription=stripBuyerQuestions(stripEventRejectionMessage(stripEventMedia(event.description)));const rejectionMessage=parseEventRejectionMessage(event.description);
 ...
 {staff.permissionSet.has("EVENT_MANAGE")&&<BuyerQuestionManager eventId={event.id} initialQuestions={buyerQuestions}/>} 
