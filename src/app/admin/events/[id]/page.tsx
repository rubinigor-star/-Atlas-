...
import { StableMapInteraction } from "@/components/stable-map-interaction";
...
export default async function ManageEvent({params,searchParams}:{params:Promise<{id:string}>;searchParams?:Promise<{tab?:string}>}){
 ...
 const map=<div className="stack">{(canManage||canTickets)&&<AdmissionModeManager eventId={event.id} initialMapEnabled={event.mapEnabled}/>} {event.mapEnabled&&canTickets?<FullscreenMapPanel><StableMapInteraction><VenueMapEditor eventId={event.id} categories={event.categories.map(category=>({id:category.id,name:category.name,priceMinor:category.priceMinor,colorHex:category.colorHex}))} initialObjects={event.zones.flatMap(zone=>zone.tables.map(item=>({id:item.id,label:item.label,objectType:item.objectType,seats:item.seats,priceMode:item.priceMode,priceMinor:item.priceMinor,x:item.x,y:item.y,rotation:item.rotation,width:item.width,height:item.height,categoryId:item.categoryId,reserved:item.reserved||item.seatItems.some(seat=>seat.status!=="AVAILABLE"),seatAssignments:item.seatItems.map(seat=>({position:seat.position,categoryId:seat.categoryId}))})))}/></StableMapInteraction></FullscreenMapPanel>:event.mapEnabled?<div className="panel">У вас нет доступа к редактированию карты.</div>:<section className="panel"><h2>Карта не используется</h2><p className="muted">Покупатель выбирает тип и количество билетов без выбора места.</p></section>}</div>;
