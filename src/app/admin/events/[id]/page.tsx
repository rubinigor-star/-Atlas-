...
export default async function ManageEvent({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <AdminShell>
      ...
      {staff.permissionSet.has("EVENT_MANAGE")&&<EventManager event={{ id: event.id, title: event.title, description: cleanDescription, media, rejectionMessage, status: event.status, startsAt: event.startsAt.toISOString(), salesMode: event.salesMode, approvalInstructions: event.approvalInstructions, mapEnabled: event.mapEnabled }} />}
