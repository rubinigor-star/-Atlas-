...
export default async function ManageEvent({ params }: { params: Promise<{ id: string }> }) {
  ...
  const event = await db.event.findUnique({ where: { id }, include: { categories: true, promoterLinks: { orderBy: { createdAt: "desc" } }, zones: { include: { tables: { include: { seatItems: true } } } } } });
  if (!event) notFound();
