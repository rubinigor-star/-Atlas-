...
export default async function ManageEvent({params}:{params:Promise<{id:string}>}){
 ...
 const event=await db.event.findUnique({where:{id},include:{venue:true,categories:true,promoterLinks:{orderBy:{createdAt:"desc"},include:{promoter:true,category:true,table:true}},zones:{include:{tables:{include:{seatItems:true}}}}}});if(!event)notFound();
