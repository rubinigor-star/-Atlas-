...
export default async function ManageEvent({ params }: { params: Promise<{ id: string }> }) {
  ...
  const event = await db.event.findUnique({ where: { id }, include: { venue: true, categories: { include: { priceTiers: true } }, promoterLinks: { orderBy: { createdAt: "desc" } }, zones: { include: { tables: { include: { seatItems: true } } } } } });
  if (!event) notFound();
  const now = new Date();
  const managedCategories = event.categories.map((category) => {
    const status = describeCategoryPrice(category, now);
    return {
      id: category.id, name: category.name, description: category.description, priceMinor: category.priceMinor,
      pricingMode: category.pricingMode, capacity: category.capacity, sold: category.sold, hidden: category.hidden,
      colorHex: category.colorHex, maxPerOrder: category.maxPerOrder,
      salesStart: category.salesStart ? category.salesStart.toISOString() : null,
      salesEnd: category.salesEnd ? category.salesEnd.toISOString() : null,
      ...
      currentPriceMinor: status.currentPriceMinor, statusLabel: status.statusLabel,
      nextTierPriceMinor: status.nextTier?.priceMinor, nextTierStartsAt: status.nextTier?.startsAt.toISOString(),
    };
  });
  ...
  const cleanDescription = stripEventRejectionMessage(stripEventMedia(event.description));
  ...
  return (
    <AdminShell>
      ...
      {staff.permissionSet.has("EVENT_MANAGE") && <GuestLinkManager eventId={event.id} categories={event.categories.map((item) => ({ id: item.id, name: item.name, priceMinor: item.priceMinor }))} tables={tables} existingLinks={event.promoterLinks.filter((item) => item.guestLimit !== null).map((item) => ({ id: item.id, label: item.label, code: item.code, guestLimit: item.guestLimit, publicPath: `/g/${item.code}`, managePath: `/g/${item.code}?token=${guestManagementToken(item.id)}` }))} />}
      {event.mapEnabled && staff.permissionSet.has("TICKET_MANAGE") && <><h2 className="section-title">Карта мероприятия</h2><VenueMapEditor eventId={event.id} categories={event.categories.map((category) => ({ id: category.id, name: category.name, priceMinor: category.priceMinor, colorHex: category.colorHex }))} initialObjects={event.zones.flatMap((zone) => zone.tables.map((item) => ({ id: item.id, label: item.label, objectType: item.objectType, seats: item.seats, priceMode: item.priceMode, priceMinor: item.priceMinor, x: item.x, y: item.y, rotation: item.rotation, width: item.width, height: item.height, categoryId: item.categoryId, reserved: item.reserved || item.seatItems.some((seat) => seat.status !== "AVAILABLE"), seatAssignments: item.seatItems.map((seat) => ({ position: seat.position, categoryId: seat.categoryId })) })))} /></>}
    </AdminShell>
  );
}
