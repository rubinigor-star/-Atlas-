...
export default async function ManageEvent({params,searchParams}:{params:Promise<{id:string}>;searchParams?:Promise<{tab?:string}>}){
 ...
 const initialTab=query.tab==="sales"||query.tab==="checkout"||query.tab==="review"?query.tab:"about";
 const about=<div className="stack">
  {canManage&&<EventTypeManager eventId={event.id} initialTypes={eventTypes}/>} 
  {canManage&&<EventLanguageManager eventId={event.id} initial={languageSettings}/>} 
  {canManage&&<EventManager event={{id:event.id,title:event.title,description:cleanDescription,posterUrl:event.posterUrl,media,rejectionMessage,status:event.status,startsAt:event.startsAt.toISOString(),salesMode:event.salesMode,approvalInstructions:event.approvalInstructions,mapEnabled:event.mapEnabled,venueName:event.venue.name,city:event.venue.city,address:event.venue.address}}/>}
 </div>;
 const sales=<div className="stack">
  {(canManage||canTickets)&&<AdmissionModeManager eventId={event.id} initialMapEnabled={event.mapEnabled}/>} 
  {canTickets&&<CategoryManager eventId={event.id} categories={managedCategories}/>} 
  {canTickets&&<PricingStrategyManager eventId={event.id} categories={event.categories.map(item=>({id:item.id,name:item.name,pricingMode:item.pricingMode,strategy:parsePricingMarketingStrategy(item.description)}))}/>} 
  {event.mapEnabled&&canTickets?<section className="panel"><span className="eyebrow">Карта мероприятия</span><h2>Схема, места и назначение билетов</h2><p className="muted">Карта является частью настройки продаж. Создавайте объекты и назначайте уже созданные категории местам прямо здесь.</p><VenueMapEditor eventId={event.id} categories={event.categories.map(category=>({id:category.id,name:category.name,priceMinor:category.priceMinor,colorHex:category.colorHex}))} initialObjects={event.zones.flatMap(zone=>zone.tables.map(item=>({id:item.id,label:item.label,objectType:item.objectType,seats:item.seats,priceMode:item.priceMode,priceMinor:item.priceMinor,x:item.x,y:item.y,rotation:item.rotation,width:item.width,height:item.height,categoryId:item.categoryId,reserved:item.reserved||item.seatItems.some(seat=>seat.status!=="AVAILABLE"),seatAssignments:item.seatItems.map(seat=>({position:seat.position,categoryId:seat.categoryId}))})))}/></section>:event.mapEnabled?<div className="panel">У вас нет доступа к редактированию карты.</div>:null}
 </div>;
 const checkout=<div className="stack">
  {canManage&&<CheckoutFormManager eventId={event.id} initialGuestFields={guestFields} initialQuestions={buyerQuestions}/>} 
  {canManage&&<EventCommercialTermsForm eventId={event.id} organizerName={event.organization.name} isSuperAdmin={staff.role==="ADMIN"} initial={{useOrganizerDefaults:commercialTerms.useOrganizerDefaults,serviceFeePayer:commercialTerms.serviceFeePayer,organizerServiceFeePayer:commercialTerms.organizer.serviceFeePayer}}/>}
  {canManage&&<GuestLinkManager eventId={event.id} categories={event.categories.map(item=>({id:item.id,name:item.name,priceMinor:item.priceMinor}))} tables={tables} existingLinks={event.promoterLinks.map(item=>{const guest=item.promoter.name.startsWith("__GUEST_LIST__")||item.promoter.name.startsWith("__CHANNEL__:GUEST:");return{id:item.id,label:item.label,code:item.code,kind:guest?"GUEST" as const:"SALES" as const,allocation:item.table?`Стол ${item.table.label}`:item.category?`Категория ${item.category.name}`:"Все билеты",priceLabel:item.customPriceMinor===0?"0 ₪":item.customPriceMinor!=null?`${item.customPriceMinor/100} ₪`:"обычная цена",limit:item.guestLimit,publicPath:guest?`/g/${item.code}`:`/events/${event.slug}?channel=${item.code}`,managePath:guest?`/g/${item.code}?token=${guestManagementToken(item.id)}`:null}})}/>} 
 </div>;
 const review=<div className="stack">
  <section className="panel"><span className="eyebrow">Проверка</span><h2>{event.title}</h2><div className="stats"><div className="stat"><span className="muted">Статус</span><strong>{archived?"ARCHIVED":event.status}</strong></div><div className="stat"><span className="muted">Формат</span><strong>{event.mapEnabled?"С выбором мест":"Без схемы"}</strong></div><div className="stat"><span className="muted">Категорий билетов</span><strong>{event.categories.length}</strong></div></div><div className="row"><Link className="btn dark" href={`/events/${event.slug}`}>Предварительный просмотр</Link>{canTickets&&<Link className="btn" href={`/office/events/${event.id}/ticket-design`}>Дизайн билета</Link>}</div></section>
  {(canManage||canTickets)&&<EventAtlasAssistant event={{id:event.id,title:event.title,status:archived?"ARCHIVED":event.status,salesMode:event.salesMode,startsAt:event.startsAt.toISOString(),venue:`${event.venue.name}, ${event.venue.city}`,categories:event.categories.map(item=>({id:item.id,name:item.name,priceMinor:item.priceMinor,capacity:item.capacity,sold:item.sold,pricingMode:item.pricingMode}))}}/>}
  {canManage&&<EventArchiveControl eventId={event.id} eventTitle={event.title} archived={archived}/>} 
 </div>;
 return <AdminShell><div className="row between"><div><span className="eyebrow">Единый редактор мероприятия</span><h1>{event.title}</h1><p className="muted">Создание, копирование и дальнейшее управление выполняются в одном интерфейсе.</p></div><span className="pill">{event.status}</span></div><EventEditorWorkspace initialTab={initialTab} about={about} sales={sales} checkout={checkout} review={review}/></AdminShell>;
}
