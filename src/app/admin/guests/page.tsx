...
export default async function GuestsPage(){
  ...
  const guests=await db.guestProfile.findMany({where:{organizationId:staff.organizationId!},include:{orders:{include:{tickets:{include:{scans:true}},event:true,promoterLink:true},orderBy:{createdAt:"desc"}}},orderBy:{updatedAt:"desc"}});
  const rows=guests.map(guest=>{
    ...
    return {"Имя":`${guest.firstName} ${guest.lastName}`,"Телефон":guest.phone,"Email":guest.email,"Возраст":age(guest.birthDate),"Город":guest.city,"Instagram":guest.instagram,"Facebook":guest.facebook,"Статус":guest.status,"Заказов":orders.length,"Посещений":visits,"Оборот":money(revenue),"Последнее мероприятие":last?.event.title??"","Последняя активность":last?new Date(last.createdAt).toLocaleDateString("ru-IL"):""};
  });
  return <AdminShell><div className="office-page-heading"><div><span className="eyebrow">Guest CRM</span><h1>Гости</h1><p>Единая база покупателей и приглашённых: контакты, покупки, посещения, источники и клиентский статус.</p></div><ExcelExportButton rows={rows} filename="atlas-guests"/></div>
