...
export async function POST(req:Request,{params}:{params:Promise<{code:string}>}){
  try{
    ...
    if(body.action==="add"){
      ...
      const category=link.category??link.table?.category??link.event.categories.find(item=>!item.hidden&&item.sold<item.capacity)??link.event.categories[0];
      if(!category)throw new Error("В мероприятии нет категории билетов для выдачи гостю");
      const used=link.orders.flatMap(order=>order.items).reduce((sum,item)=>sum+item.quantity,0);const limit=link.guestLimit??link.table?.seats??category.capacity;if(used>=limit)throw new Error("Лимит гостей исчерпан");if(category.sold>=category.capacity)throw new Error("Билеты этой категории закончились");
    ...
    if(body.action==="remove"){const input=removeSchema.parse(body);const order=link.orders.find(item=>item.id===input.orderId);if(!order)throw new Error("Гость не найден");if(order.tickets.some(ticket=>ticket.status==="USED"))throw new Error("Нельзя удалить гостя после прохода");const quantity=order.items.reduce((sum,item)=>sum+item.quantity,0);const category=link.category??link.table?.category??link.event.categories.find(item=>item.name===order.items[0]?.categoryName);await db.$transaction(async tx=>{await tx.ticket.updateMany({where:{orderId:order.id},data:{status:"CANCELLED"}});await tx.order.update({where:{id:order.id},data:{status:"CANCELLED"}});if(category)await tx.ticketCategory.update({where:{id:category.id},data:{sold:{decrement:quantity}}});});return NextResponse.json({ok:true});}
    throw new Error("Неизвестное действие");
  }catch(error){const message=error instanceof Error?error.message:"Ошибка";console.error("GUEST_LIST_REQUEST_FAILED",message);return NextResponse.json({error:message},{status:400});}
}
