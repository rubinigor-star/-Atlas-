...
export async function GET(request: NextRequest) {
  ...
  if (!organization) return NextResponse.json({ error: "Malina organization not found", organizations }, { status: 404 });
  ...
  if (!event) {
    event = await db.event.create({
      data: {
        slug: SLUG,
        title: "Scanner Test",
        description: "Scanner Test - temporary event for QR scanner validation",
        posterUrl: latestEvent.posterUrl || "",
        startsAt: new Date("2026-08-20T08:50:00.000Z"),
        salesStart: new Date("2026-08-20T08:45:00.000Z"),
        salesEnd: new Date("2026-08-20T14:00:00.000Z"),
        status: "PUBLISHED",
        salesMode: "INSTANT",
  ...
  if (!category) {
    category = await db.ticketCategory.create({
      data: {
        eventId: event.id,
        name: "Scanner Test",
        description: "10 QR tickets for scanner testing",
        priceMinor: 0,
        currency: "ILS",
        capacity: 10,
        sold: 10,
  ...
  if (!order) {
    order = await db.order.create({
      data: {
        publicId: `ATL-SCANNER-${Date.now()}`,
        idempotencyKey: `scanner-test-${Date.now()}`,
        ...
        eventId: event.id,
        items: {
          create: [{ quantity: 10, unitPriceMinor: 0, categoryName: category.name }],
        },
      },
    });
  }
  ...
  return NextResponse.json({
    ...
    event: { id: event.id, slug: event.slug, title: event.title, startsAt: event.startsAt, salesEnd: event.salesEnd },
    venue: latestEvent.venue,
    order: { id: order.id, publicId: order.publicId, email: order.customerEmail },
    tickets: tickets.map(t => ({ id: t.id, publicCode: t.publicCode, status: t.status })),
    emailResult,
  });
}
