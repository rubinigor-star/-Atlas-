...
async function getValueCard(organizationId: string) {
  ...
  const rows = await db.$queryRaw<IntegrationRow[]>`
    SELECT "id", "enabled", "credentialsEncrypted"
...
export async function GET() {
  try {
    const actor = await requirePermission("FINANCE_VIEW");
    const integration = await getValueCard(actor.organizationId!);
    return NextResponse.json({ enabled: Boolean(integration?.enabled), configured: Boolean(integration?.credentialsEncrypted) });
  } catch (error) {
...
export async function PUT(request: Request) {
  try {
    ...
    const message = error instanceof Error ? error.message : "Ошибка";
    const friendly = message === "INTEGRATIONS_ENCRYPTION_KEY_NOT_CONFIGURED" ? "На сервере не настроен INTEGRATIONS_ENCRYPTION_KEY" : message === "FORBIDDEN" ? "Недостаточно прав" : message;
