...
export async function POST(req: Request) {
  try {
    ...
    const result = await db.$transaction(async (tx) => {
      ...
      const claimed = await tx.ticket.updateMany({ where: { id: visible.id, status: "VALID" }, data: { status: "USED",walletUpdatedAt:new Date() } });
    ...
    if(result.status==="VALID"&&visible)await notifyWalletTickets([visible.id]);
