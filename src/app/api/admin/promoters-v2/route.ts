...
import { assignPromoterV2, createPromoterV2, getPromoterV2, listAssignmentsV2, listPromotersV2, setPromoterV2Active, setPromoterV2Automation } from "@/lib/promoter-v2";
...
export async function POST(req:Request){
 try{
  ...
  if(body.action==="create"){
   ...
   const assignments=[];for(const eventId of input.eventIds){assignments.push(await assignPromoterV2(promoter.id,eventId));}
   return NextResponse.json({ok:true,promoter,assignments},{status:201});
  }
  if(body.action==="assign"){
   const input=assignSchema.parse(body);const {promoter}=await ownPromoter(input.promoterId);await requireEventAccess("EVENT_MANAGE",input.eventId);const assignment=await assignPromoterV2(promoter.id,input.eventId);return NextResponse.json({ok:true,assignment},{status:201});
  }
