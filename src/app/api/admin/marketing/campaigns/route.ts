...
export async function PATCH(req:Request){
  try{
    ...
    if(input.action==="archive"){
      ...
      await db.$executeRawUnsafe(`UPDATE MarketingCampaign SET status='ARCHIVED',updatedAt=CURRENT_TIMESTAMP WHERE id=? AND organizationId=?`,campaign.id,actor.organizationId);
