  ...
  try{
    ...
    const result=await db.$transaction(async tx=>{
      ...
      for(const item of items){
        ...
        const itemCategoryQuantities=new Map<string,{name:string;minPerOrder:number;maxPerOrder:number;quantity:number}>();
        const addItemCategoryQuantity=(selectedCategory:{id:string;name:string;minPerOrder:number;maxPerOrder:number},amount:number)=>{const current=itemCategoryQuantities.get(selectedCategory.id);itemCategoryQuantities.set(selectedCategory.id,{name:selectedCategory.name,minPerOrder:selectedCategory.minPerOrder,maxPerOrder:selectedCategory.maxPerOrder,quantity:(current?.quantity??0)+amount});};
        if(item.tableId){
