...
export async function POST(req: Request) {
  try {
    ...
    const order = await db.$transaction(async (tx) => {
      ...
      const promoterLink = input.referralCode ? await tx.promoterLink.findUnique({ where: { code: input.referralCode.toUpperCase() }, include: { orders: { where: { status: { notIn: ["CANCELLED", "REJECTED"] } }, include: { items: true } } } }) : null;
      ...
      else if (seats.length) for (const seat of seats) { const assigned = seat.category!; const current = requested.get(assigned.id); requested.set(assigned.id, { category: assigned, quantity: (current?.quantity ?? 0) + 1, price: effectiveTicketPrice(assigned) }); }
      ...
      let discount = 0;
      if (input.promoCode) { const promo = await tx.promoCode.findUnique({ where: { eventId_code: { eventId: input.eventId, code: input.promoCode.toUpperCase() } } }); if (promo?.active) discount = promo.discountPercent; }
      const standardSubtotal = [...requested.values()].reduce((sum, item) => sum + item.price * (table ? 1 : item.quantity), 0);
      const promoterSubtotal = promoterLink?.customPriceMinor !== null && promoterLink?.customPriceMinor !== undefined ? (promoterLink.allocationType === "TABLE" ? promoterLink.customPriceMinor : promoterLink.customPriceMinor * quantity) : null;
      const subtotal = promoterSubtotal ?? standardSubtotal;
      const total = Math.round((subtotal * (100 - discount)) / 100);
      const phone=normalizePhone(input.customer.phone);
      const fullName=`${input.customer.firstName.trim()} ${input.customer.lastName.trim()}`;
      const guest=await tx.guestProfile.upsert({
        where:{organizationId_phone:{organizationId:event.organizationId,phone}},
        create:{organizationId:event.organizationId,firstName:input.customer.firstName.trim(),lastName:input.customer.lastName.trim(),phone,email:input.customer.email.toLowerCase(),birthDate:new Date(input.customer.birthDate),city:input.customer.city,facebook:input.customer.facebook,instagram:input.customer.instagram},
        update:{firstName:input.customer.firstName.trim(),lastName:input.customer.lastName.trim(),email:input.customer.email.toLowerCase(),birthDate:new Date(input.customer.birthDate),city:input.customer.city,facebook:input.customer.facebook,instagram:input.customer.instagram}
      });

      const created = await tx.order.create({ data: {
        publicId: orderNumber(), idempotencyKey: input.idempotencyKey, customerName: fullName, customerEmail: input.customer.email.toLowerCase(), customerPhone: phone,
        customerFirstName:input.customer.firstName.trim(),customerLastName:input.customer.lastName.trim(),customerBirthDate:new Date(input.customer.birthDate),customerCity:input.customer.city,customerFacebook:input.customer.facebook,customerInstagram:input.customer.instagram,
        guestId:guest.id, eligibilityAnswer: input.eligibilityAnswer || null, totalMinor: total, status: initialOrderStatus(event.salesMode), eventId: input.eventId, referralId: legacyReferral?.id, promoterLinkId: promoterLink?.id,
        items: { create: seats.length ? seats.map((seat) => ({ quantity: 1, unitPriceMinor: promoterLink?.customPriceMinor ?? effectiveTicketPrice(seat.category!), categoryName: seat.category!.name, tableId: seat.tableId, seatId: seat.id })) : [{ quantity, unitPriceMinor: promoterLink?.customPriceMinor !== null && promoterLink?.customPriceMinor !== undefined ? (promoterLink.allocationType === "TABLE" ? Math.round(promoterLink.customPriceMinor / quantity) : promoterLink.customPriceMinor) : table?.category ? Math.round(effectiveTicketPrice(table.category) / quantity) : categoryPrice, categoryName: table?.category?.name ?? category.name, tableId: table?.id }] },
        tickets: event.salesMode === "INSTANT" ? { create: seats.length ? seats.map((seat) => ({ publicCode: ticketCode(), holderName: fullName, categoryId: seat.category!.id })) : Array.from({ length: quantity }, () => ({ publicCode: ticketCode(), holderName: fullName, categoryId: table?.category?.id ?? category.id })) } : undefined,
      }});
      ...
      if (event.salesMode === "INSTANT" && table) { const claimed = await tx.table.updateMany({ where: { id: table.id, reserved: false }, data: { reserved: true } }); if (claimed.count !== 1) throw new Error("Этот стол только что был забронирован"); }
      if (event.salesMode === "INSTANT" && seats.length) { const claimed = await tx.seat.updateMany({ where: { id: { in: seats.map((seat) => seat.id) }, status: "AVAILABLE" }, data: { status: "RESERVED" } }); if (claimed.count !== seats.length) throw new Error("Одно из мест только что было забронировано"); }
      return created;
    });
