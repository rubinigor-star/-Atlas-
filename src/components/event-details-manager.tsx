...
import { Clock3, MapPin, ShieldCheck, Theater } from "lucide-react";
...
type Presentation = { shortDescription:string; ageRestriction:string; doorsOpenTime:string; galleryEnabled:boolean; galleryUrls:string[]; faqEnabled:boolean; faq:EventFaqItem[] };
...
function encodePresentation(value:Presentation){const faq=value.faq.map(i=>({question:i.question.trim().slice(0,180),answer:i.answer.trim().slice(0,1200)})).filter(i=>i.question||i.answer).slice(0,MAX_FAQ_ITEMS);const normalized={...value,shortDescription:value.shortDescription.trim().slice(0,SHORT_DESCRIPTION_LIMIT),ageRestriction:ageRestrictionOptions.includes(value.ageRestriction as never)?value.ageRestriction:"Без ограничений",galleryEnabled:value.galleryEnabled&&value.galleryUrls.length>0,galleryUrls:value.galleryUrls.slice(0,6),faqEnabled:value.faqEnabled&&faq.some(i=>i.question&&i.answer),faq};const bytes=new TextEncoder().encode(JSON.stringify(normalized));let binary="";for(const byte of bytes)binary+=String.fromCharCode(byte);return`<!--ATLAS_EVENT_PRESENTATION:${window.btoa(binary)}-->`;}
