...
export function EventDetailsManager({event}:{event:EventDetails}){
 const router=useRouter();const{locale}=useLocale();const text=labels[locale];const[message,setMessage]=useState("");const[shortDescription,setShortDescription]=useState(event.shortDescription||"");
 async function submit(form:HTMLFormElement){
  ...
  const cleanDescription=String(f.get("description")||"").replace(/\n?<!--ATLAS_EVENT_PRESENTATION:[A-Za-z0-9+/=]+-->/g,"").trim();const marker=encodePresentation(shortDescription);const description=marker?`${cleanDescription}\n${marker}`:cleanDescription;
  setMessage("");
