...
type ActionResponse={error?:string;emailSent?:boolean;emailError?:string;publicId?:string};
...
export function GuestListPage({code,token,title,eventTitle,allocation,limit,guests,canManage,fields}:{code:string;token:string;title:string;eventTitle:string;allocation:string;limit:number;guests:Guest[];canManage:boolean;fields:GuestFieldConfig}){
  ...
  async function send(body:Record<string,unknown>){
    ...
    try{
      ...
      if(body.action==="add"){
        if(data.emailSent)setSuccess(`Гость добавлен. Билет ${data.publicId||""} отправлен по email.`);
        else setError(`Гость добавлен, но письмо не отправлено${data.emailError?`: ${data.emailError}`:"."}`);
  ...
  return <main className="shell"><section className="panel" style={{maxWidth:720,margin:"28px auto"}}><span className="eyebrow">ATLAS GUEST LIST</span><h1>{title}</h1><p><strong>{eventTitle}</strong><br/><span className="muted">{allocation}</span></p><div className="stats"><div className="stat"><span className="muted">Записано</span><strong>{guests.length}</strong></div><div className="stat"><span className="muted">Осталось</span><strong>{Math.max(0,limit-guests.length)}</strong></div></div>
    {canManage&&guests.length<limit&&<form className="form panel" onSubmit={async e=>{e.preventDefault();const form=e.currentTarget;const f=new FormData(form);const customer=Object.fromEntries(visible.map(key=>[key,String(f.get(key)||"")]));const ok=await send({action:"add",token,customer});if(ok)form.reset();}}><h2>Добавить гостя</h2><p className="muted">После добавления Atlas автоматически создаст PDF-билет и отправит его на указанный email.</p>{visible.map(key=>{const required=key==="email"||fields[key]?.required;return <div className="field" key={key}><label>{labels[key]}{required?" *":" — необязательно"}</label><input className="input" name={key} type={types[key]||"text"} required={required} disabled={busy} autoComplete={key==="firstName"?"given-name":key==="lastName"?"family-name":key==="phone"?"tel":key==="email"?"email":"off"}/></div>;})}{fields.birthDate.visible&&<p className="muted">Возраст будет рассчитан автоматически по дате рождения.</p>}{error&&<div className="toast" role="alert">{error}</div>}{success&&<div className="toast" role="status">{success}</div>}<button className="btn" disabled={busy}>{busy?"Создаём и отправляем...":"Добавить гостя и отправить билет"}</button></form>}
