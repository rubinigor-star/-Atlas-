    ...
    <form onSubmit={submit} className={`checkout ${busy ? "loading" : ""}`}>
      <section>
        ...
        <div className="panel form">
          ...
          <div className="field"><label>{text.name}</label><input className="input" name="name" required minLength={2} autoComplete="name" /></div>
          <div className="field"><label>{text.phone}</label><input className="input" name="phone" required autoComplete="tel" placeholder="+972..." /></div>
          ...
          <button className="btn" disabled={busy}>{busy ? text.sending : approvalRequired ? text.send : text.confirm}</button>
        </div>
      </section>
      <aside className="panel summary">
        ...
        <div className="row between"><strong>{approvalRequired ? text.afterApproval : text.total}</strong><strong style={{ fontSize: 25 }}>{money(props.total)}</strong></div>
      </aside>
    </form>
  );
}
