...
import { ArrowLeft, ChevronDown, Menu, Minus, Plus, RotateCcw, UsersRound } from "lucide-react";
...
export function EventSeatSelection({ eventId, slug, title, posterUrl, venueName, categories, objects, feeTerms, referralCode, allocation, initialQty }: { eventId:string; slug:string; title:string; posterUrl:string; venueName:string; categories:Category[]; objects:MapObject[]; feeTerms:ServiceFeeTerms; referralCode?:string; allocation?:Allocation; initialQty:number }) {
  ...
  const local={
    ru:{back:"Вернуться в мероприятие",offers:"Специальное предложение",all:"Все билеты",onePlusOne:"1 + 1",people:"Количество билетов",peopleHelp:"Выберите размер группы. На карте останутся только места, где вся группа сможет сидеть рядом.",selected:"Выбрано",continue:"Продолжить",price:"Цена",noSeats:"В выбранном диапазоне нет подходящих мест рядом",zoomReset:"Сбросить масштаб"},
    en:{back:"Back to event",offers:"Special offer",all:"All tickets",onePlusOne:"1 + 1",people:"Ticket quantity",peopleHelp:"Choose your group size. The map will only show seats where everyone can sit together.",selected:"Selected",continue:"Continue",price:"Price",noSeats:"No adjacent seats match this price range",zoomReset:"Reset zoom"},
    he:{back:"חזרה לאירוע",offers:"הצעה מיוחדת",all:"כל הכרטיסים",onePlusOne:"1 + 1",people:"כמות כרטיסים",peopleHelp:"בחרו את גודל הקבוצה. במפה יוצגו רק מקומות שבהם כולם יכולים לשבת יחד.",selected:"נבחרו",continue:"המשך",price:"מחיר",noSeats:"אין מקומות צמודים בטווח המחירים שנבחר",zoomReset:"איפוס זום"},
  }[locale];
  ...
  const rawSubtotal=zoneObject ? (categories.find(item=>item.id===zoneObject.categoryId)?.priceMinor??zoneObject.priceMinor)*qty : wholeObject ? (allocation?.type==="TABLE"&&allocation.customPriceMinor!==null?allocation.customPriceMinor:(categories.find(item=>item.id===wholeObject.categoryId)?.priceMinor??wholeObject.priceMinor)) : selectedSeats.reduce((sum,seat)=>sum+(categories.find(item=>item.id===seat.categoryId)?.priceMinor??0),0);
  ...
  function setPeople(next:number){setQty(Math.max(1,Math.min(8,next)));setPeopleOpen(false);clearSelection();}
  function chooseOffer(next:OfferFilter){setOffer(next);setOfferOpen(false);if(next==="BUY_ONE_GET_ONE")setQty(value=>value<2?2:value%2===0?value:Math.min(8,value+1));clearSelection();}
  function chooseSeat(object:MapObject,seat:MapSeat){
    if(!eligibleSeatIds.has(seat.id)||seat.status!=="AVAILABLE")return;
    setWholeObjectId(null);setZoneObjectId(null);
  ...
  function chooseZone(object:MapObject){
    ...
    setSelectedSeatIds([]);setWholeObjectId(null);setZoneObjectId(current=>current===object.id?null:object.id);
  }
  function go(){
  ...
  function startPan(event:React.PointerEvent<HTMLDivElement>){
    ...
    viewport.setPointerCapture(event.pointerId);setPanning(true);event.preventDefault();
  }
  function movePan(event:React.PointerEvent<HTMLDivElement>){
    ...
    viewport.scrollLeft=panRef.current.left-(event.clientX-panRef.current.x);viewport.scrollTop=panRef.current.top-(event.clientY-panRef.current.y);
  }
  function endPan(event:React.PointerEvent<HTMLDivElement>){
    if(panRef.current.pointerId!==event.pointerId)return;
    const viewport=viewportRef.current;if(viewport?.hasPointerCapture(event.pointerId))viewport.releasePointerCapture(event.pointerId);
    panRef.current.pointerId=-1;setPanning(false);
  }
  ...
  return <main className={styles.page}>
    <Link className={styles.headerBack} href={backHref}><ArrowLeft size={18}/><span>{local.back}</span></Link>
    <div className={styles.layout}>
      <section className={styles.mapSide}>
        <div className={styles.priceRail}>
          <div className={styles.priceStops} style={{gridTemplateColumns:`repeat(${Math.max(1,sortedPrices.length)},minmax(0,1fr))`}}>{sortedPrices.map((price,index)=>{const category=availableCategories.find(item=>(categoryPrice.get(item.id)??0)===price);const active=index>=minIndex&&index<=maxIndex;return <button type="button" key={`${price}-${index}`} className={active?styles.priceActive:""} onClick={()=>{if(index<minIndex)setMinIndex(index);else if(index>maxIndex)setMaxIndex(index);}}><b style={{color:category?.colorHex??"#64748b"}}>{money(price,"ILS",locale)}</b></button>;})}</div>
          <div className={styles.rangeWrap}><div className={styles.rangeBase}/><div className={styles.rangeActive} style={{left:`${activeLeft}%`,width:`${activeWidth}%`}}/><input aria-label="minimum ticket price" className={styles.range} type="range" min="0" max={Math.max(0,sortedPrices.length-1)} value={minIndex} onChange={event=>{setMinIndex(Math.min(Number(event.target.value),maxIndex));clearSelection();}}/><input aria-label="maximum ticket price" className={`${styles.range} ${styles.rangeMax}`} type="range" min="0" max={Math.max(0,sortedPrices.length-1)} value={maxIndex} onChange={event=>{setMaxIndex(Math.max(Number(event.target.value),minIndex));clearSelection();}}/></div>
        </div>
          ...
          <div className={styles.mapFrame} style={{width:WORLD_WIDTH*scale,height:WORLD_HEIGHT*scale}}><div className={styles.world} style={{width:WORLD_WIDTH,height:WORLD_HEIGHT,transform:`scale(${scale})`}}>
            {availableObjects.map(object=>{
              ...
              return <div key={object.id} className={`${styles.object} ${zone?styles.zoneLayer:""}`} style={{left:`${object.x}%`,top:`${object.y}%`,width:object.width,height:object.height,transform:`translate(-50%,-50%) rotate(${object.rotation}deg)`,opacity:faded?.12:1,pointerEvents:faded?"none":undefined}}>
                {!seatObject ? <div className={`${styles.decoration} ${styles[`decoration${object.objectType}`]??""} ${selectedZone?styles.zoneSelected:""}`} style={zoneColor?{background:zoneColor,borderColor:zoneColor,color:"#fff",boxShadow:`inset 0 0 0 1px rgba(255,255,255,.18)`}:undefined} onClick={()=>chooseZone(object)}><strong>{object.label}</strong></div> : <div className={`${styles.furniture} ${styles[`furniture${object.objectType}`]??""}`} onClick={()=>{if(object.priceMode!=="WHOLE_TABLE"||object.reserved||!wholeVisible)return;setSelectedSeatIds([]);setZoneObjectId(null);setWholeObjectId(selectedWhole?null:object.id);}}><div className={`${styles.core} ${selectedWhole?styles.coreSelected:""}`}><strong>{object.label}</strong></div>{object.objectType==="ROW"?<div className={styles.rowSeats}>{object.seatItems.map(seat=>{const categoryId=seat.categoryId??object.categoryId;const color=categories.find(item=>item.id===categoryId)?.colorHex??"#e3e7eb";const eligible=eligibleSeatIds.has(seat.id);return <SeatDot key={seat.id} seat={seat} object={object} color={seat.status==="AVAILABLE"?color:"#e3e7eb"} selected={selectedSeatIds.includes(seat.id)} eligible={eligible} disabled={object.priceMode==="WHOLE_TABLE"||!eligible} onClick={()=>chooseSeat(object,seat)}/>;})}</div>:object.seatItems.map(seat=>{const categoryId=seat.categoryId??object.categoryId;const color=categories.find(item=>item.id===categoryId)?.colorHex??"#e3e7eb";const eligible=eligibleSeatIds.has(seat.id);return <SeatDot key={seat.id} seat={seat} object={object} color={seat.status==="AVAILABLE"?color:"#e3e7eb"} selected={selectedSeatIds.includes(seat.id)} eligible={eligible} disabled={object.priceMode==="WHOLE_TABLE"||!eligible} onClick={()=>chooseSeat(object,seat)}/>;})}</div>}
              </div>;
            })}
          </div></div>
      ...
      <aside className={styles.sidebar}>
        <div className={styles.eventInfo}><img src={posterUrl} alt=""/><div className={styles.eventDetails}><h1>{title}</h1><div className={styles.eventPills}>
          <div className={styles.offerWrap}><button type="button" className={styles.offerButton} onClick={()=>{setOfferOpen(value=>!value);setPeopleOpen(false);}}><Menu size={17}/><span>{offer==="ALL"?local.offers:local.onePlusOne}</span><ChevronDown size={15}/></button>{offerOpen&&<div className={styles.offerMenu}><button type="button" className={offer==="ALL"?styles.offerSelected:""} onClick={()=>chooseOffer("ALL")}>{local.all}</button>{hasOnePlusOne&&<button type="button" className={offer==="BUY_ONE_GET_ONE"?styles.offerSelected:""} onClick={()=>chooseOffer("BUY_ONE_GET_ONE")}>{local.onePlusOne}</button>}</div>}</div>
          <div className={styles.peopleWrap}><button type="button" className={styles.peopleButton} onClick={()=>{setPeopleOpen(value=>!value);setOfferOpen(false);}}><UsersRound size={17}/><strong>{qty}</strong><ChevronDown size={15}/></button>{peopleOpen&&<div className={styles.peopleMenu}><strong>{local.people}</strong><p>{local.peopleHelp}</p><div className={styles.peopleGrid}>{Array.from({length:8},(_,index)=>index+1).map(value=><button type="button" key={value} className={qty===value?styles.peopleSelected:""} onClick={()=>setPeople(value)}>{value}</button>)}</div></div>}</div>
        </div></div></div>
