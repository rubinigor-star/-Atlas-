...
export function VenueMapEditor({ eventId, categories, initialObjects }: { eventId: string; categories: Category[]; initialObjects: MapObject[] }) {
  ...
  return <section className="venue-builder">
    ...
    <div className={`builder-body ${inspector ? "inspector-open" : ""}`}>
      ...
      <main className="builder-workspace"><div className="floating-tools"><button type="button" className="tool-active">↖</button><span/><button type="button" onClick={() => setZoom((value) => Math.max(35, value - 10))}>−</button><strong>{zoom}%</strong><button type="button" onClick={() => setZoom((value) => Math.min(125, value + 10))}>＋</button></div><div className="workspace-hint">{mode === "design" ? text.designHelp : text.ticketHelp}</div>{mode === "tickets" && <div className="ticket-legend"><strong>{text.legend}</strong>{categories.map((category) => <span key={category.id}><i style={{ background:category.colorHex }}/>{category.name} · {money(category.priceMinor)}</span>)}<span><i className="unassigned"/>{text.unassigned}</span></div>}<div className="map-scroll"><div className="map-world-frame" style={{ width:1000 * scale, height:700 * scale }}><div className={`map-world ${mode}`} style={{ transform:`scale(${scale})` }} onClick={() => { setSelectedId(""); setInspector(false); setSelectedPositions(new Set()); }}>{objects.map((item) => <div key={item.id} className={`editor-object ${selectedId === item.id ? "selected" : ""} ${item.reserved ? "locked" : ""}`} style={{ left:`${item.x}%`, top:`${item.y}%`, width:item.width, height:item.height, transform:`translate(-50%,-50%) rotate(${item.rotation}deg)`, zIndex:item.objectType === "ZONE" ? 1 : 2 }} onClick={(event) => { event.stopPropagation(); choose(item.id); }} onPointerDown={(event) => { if (mode !== "design" || item.reserved || event.button !== 0) return; event.stopPropagation(); drag.current = { id:item.id, before:objects }; try { event.currentTarget.setPointerCapture(event.pointerId); } catch { drag.current = null; } }} onPointerMove={(event) => { if (drag.current?.id !== item.id) return; const world = event.currentTarget.parentElement; if (!world) return; const bounds = world.getBoundingClientRect(); if (!bounds.width || !bounds.height) return; setObjects((current) => current.map((value) => value.id === item.id ? { ...value, x:clamp((event.clientX - bounds.left) / bounds.width * 100), y:clamp((event.clientY - bounds.top) / bounds.height * 100) } : value)); }} onPointerUp={(event) => { if (drag.current?.id === item.id) { setHistory((current) => [...current.slice(-29), drag.current!.before]); setFuture([]); drag.current = null; try { if (event.currentTarget.hasPointerCapture(event.pointerId)) event.currentTarget.releasePointerCapture(event.pointerId); } catch { /* element may have moved during fullscreen layout update */ } } }} onPointerCancel={(event) => { if (drag.current?.id === item.id) drag.current = null; try { if (event.currentTarget.hasPointerCapture(event.pointerId)) event.currentTarget.releasePointerCapture(event.pointerId); } catch { /* no-op */ } }}><Shape item={item} categories={categories} mode={mode} selectedSeats={selectedId === item.id ? selectedPositions : new Set()} onSeat={selectSeat}/></div>)}</div></div></div><div className="builder-status"><span>{text.capacity}: {capacity}</span><span>{message}</span></div></main>
