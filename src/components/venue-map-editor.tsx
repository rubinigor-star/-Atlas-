...
function prepareInitialObjects(initialObjects: MapObject[], venueName?: string): MapObject[] {
  ...
  return readingVenuePreset().map((presetItem) => {
    ...
    if (!previous || !sellable.has(presetItem.objectType)) return presetItem;
    return {
      ...presetItem,
...
function tableSeatPosition(item: MapObject, index: number): React.CSSProperties {
  ...
  if (item.seats === 6 && horizontal) {
    const top = index < 3;
    const slot = index % 3;
    return { left:`${[18, 50, 82][slot]}%`, top:top ? "14%" : "86%" };
  }

  if (item.seats === 8 && !horizontal) {
    const left = index < 4;
    const slot = index % 4;
    return { left:left ? "15%" : "85%", top:`${[14, 38, 62, 86][slot]}%` };
  }
  ...
  return horizontal
    ? { left:offset, top:firstSide ? "14%" : "86%" }
    : { left:firstSide ? "15%" : "85%", top:offset };
}
...
function Shape({ item, categories, mode, selectedSeats, onSeat }: { item: MapObject; categories: Category[]; mode: "design" | "tickets"; selectedSeats: Set<number>; onSeat: (position: number, shift: boolean) => void }) {
  ...
  if (item.objectType === "ROW") {
    chairs = <span className="row-chairs">{Array.from({ length: item.seats }, (_, index) => seat(index + 1, "row-chair"))}</span>;
  } else if (item.objectType === "SOFA") {
    chairs = <span className="sofa-cushions">{Array.from({ length: item.seats }, (_, index) => seat(index + 1, "sofa-seat", { left:`${((index + 1) / (item.seats + 1)) * 100}%`, top:"72%" }))}</span>;
  } else if (item.objectType === "ROUND_TABLE") {
    chairs = Array.from({ length: item.seats }, (_, index) => {
      const angle = index / item.seats * Math.PI * 2 - Math.PI / 2;
      return seat(index + 1, "radial-chair", { left:`${50 + Math.cos(angle) * 39}%`, top:`${50 + Math.sin(angle) * 39}%` });
    });
  } else {
    chairs = Array.from({ length: item.seats }, (_, index) => seat(index + 1, "edge-chair", tableSeatPosition(item, index)));
  }
...
export function VenueMapEditor({ eventId, categories, initialObjects, venueName }: { eventId: string; categories: Category[]; initialObjects: MapObject[]; venueName?: string }) {
  ...
  function add(type: ObjectType, labelOverride?: string) {
    ...
    const item: MapObject = { id:`new-${crypto.randomUUID()}`, label:labelOverride ?? `${names[type]}${sellable.has(type) ? ` ${number}` : ""}`, objectType:type, ...base, priceMinor:0, x:50, y:50, categoryId:null, reserved:false, seatAssignments:assignments(base.seats, []) };
  ...
  return <section className={`venue-builder ${mapStyle.atlas}`}>
    ...
    <div className={`builder-body ${inspector ? "inspector-open" : ""}`}>
      ...
      <main className="builder-workspace"><div className="floating-tools"><button type="button" className="tool-active">↖</button><span/><button type="button" onClick={() => setZoom((value) => Math.max(35, value - 10))}>−</button><strong>{zoom}%</strong><button type="button" onClick={() => setZoom((value) => Math.min(125, value + 10))}>＋</button></div><div className="workspace-hint">{mode === "design" ? text.designHelp : text.ticketHelp}</div>{mode === "tickets" && <div className="ticket-legend"><strong>{text.legend}</strong>{categories.map((category) => <span key={category.id}><i style={{ background:category.colorHex }}/>{category.name} · {money(category.priceMinor)}</span>)}<span><i className="unassigned"/>{text.unassigned}</span></div>}<div className="map-scroll"><div className="map-world-frame" style={{ width:WORLD_WIDTH * scale, height:WORLD_HEIGHT * scale }}><div className={`map-world ${mode}`} style={{ width:WORLD_WIDTH, height:WORLD_HEIGHT, transform:`scale(${scale})` }} onClick={() => { setSelectedId(""); setInspector(false); setSelectedPositions(new Set()); }}>{objects.filter((item) => !isPresetMarker(item)).map((item) => <div key={item.id} className={`editor-object ${selectedId === item.id ? "selected" : ""} ${item.reserved ? "locked" : ""}`} style={{ left:`${item.x}%`, top:`${item.y}%`, width:item.width, height:item.height, transform:`translate(-50%,-50%) rotate(${item.rotation}deg)`, zIndex:item.objectType === "ZONE" ? 1 : item.objectType === "STAGE" || item.objectType === "BAR" ? 2 : 3 }} onClick={(event) => { event.stopPropagation(); choose(item.id); }} onPointerDown={(event) => { if (mode !== "design" || item.reserved || event.button !== 0) return; event.stopPropagation(); drag.current = { id:item.id, before:objects }; try { event.currentTarget.setPointerCapture(event.pointerId); } catch { drag.current = null; } }} onPointerMove={(event) => { if (drag.current?.id !== item.id) return; const world = event.currentTarget.parentElement; if (!world) return; const bounds = world.getBoundingClientRect(); if (!bounds.width || !bounds.height) return; setObjects((current) => current.map((value) => value.id === item.id ? { ...value, x:clamp((event.clientX - bounds.left) / bounds.width * 100), y:clamp((event.clientY - bounds.top) / bounds.height * 100) } : value)); }} onPointerUp={(event) => { if (drag.current?.id === item.id) { setHistory((current) => [...current.slice(-29), drag.current!.before]); setFuture([]); drag.current = null; try { if (event.currentTarget.hasPointerCapture(event.pointerId)) event.currentTarget.releasePointerCapture(event.pointerId); } catch { /* element may have moved during fullscreen layout update */ } } }} onPointerCancel={(event) => { if (drag.current?.id === item.id) drag.current = null; try { if (event.currentTarget.hasPointerCapture(event.pointerId)) event.currentTarget.releasePointerCapture(event.pointerId); } catch { /* no-op */ } }}><Shape item={item} categories={categories} mode={mode} selectedSeats={selectedId === item.id ? selectedPositions : new Set()} onSeat={selectSeat}/></div>)}</div></div></div><div className="builder-status"><span>{text.capacity}: {capacity}</span><span>{message}</span></div></main>
