...
function AtlasOneLogo() {
  return <span className="atlas-one-logo"><b>ATL<span>AS</span></b><em>ONE</em></span>;
}
...
export async function AdminShell({ children }: { children: React.ReactNode }) {
  ...
  return <div className="office-shell">
    <aside className="office-sidebar"><Link className="office-brand" href="/" aria-label="Atlas One — главная страница"><AtlasOneLogo /></Link><div className="office-org"><i>{staff.organization.name.slice(0,1)}</i><div><strong>{staff.organization.name}</strong><small>{roleLabels[staff.staffRole??"CUSTOM"]}</small></div></div><OfficeNavigation permissions={[...staff.permissionSet]}/><OfficeAccountMenu currentEmail={staff.email} currentName={staff.name} colleagues={accounts}/></aside>
    <main className="office-main"><header className="office-mobile-header"><Link className="office-brand" href="/" aria-label="Atlas One — главная страница"><AtlasOneLogo /></Link><OfficeAccountMenu compact currentEmail={staff.email} currentName={staff.name} colleagues={accounts}/></header>{children}</main>
