...
export function EventEditorWorkspace({about,tickets,map,checkout,review,initialTab="about"}:{about:ReactNode;tickets:ReactNode;map:ReactNode;checkout:ReactNode;review:ReactNode;initialTab?:TabId}){
 ...
 return <div className={styles.shell}>
  ...
  <div className={styles.content}>
   ...
   <section className={active==="tickets"?"":styles.hidden}>{tickets}</section>
   <section className={active==="map"?"":styles.hidden}>{map}</section>
  ...
  <div className={styles.footer}><button type="button" className={styles.secondary} disabled={index===0} onClick={()=>open(tabs[index-1].id)}>Назад</button>{index<tabs.length-1?<button type="button" className="btn" onClick={()=>open(tabs[index+1].id)}>Далее</button>:<span className="muted">Проверьте настройки и откройте предварительный просмотр перед публикацией.</span>}</div>
 </div>;
}
