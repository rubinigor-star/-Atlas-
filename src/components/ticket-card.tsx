/* eslint-disable @next/next/no-img-element -- organizer-provided local assets use intrinsic canvas sizing */
import Image from "next/image";
import Link from "next/link";
import { WalletCards } from "lucide-react";
import { AtlasLogo } from "@/components/atlas-logo";
import type { TicketDesign, TicketElement } from "@/lib/ticket-template-types";
import { formatTicketDate, formatTicketTime, getTicketLocale } from "@/lib/ticket-language";

type Data = {
  eventTitle: string;
  date: string;
  time: string;
  venue: string;
  address: string;
  customerName: string;
  ticketType: string;
  orderNumber: string;
  ticketCode: string;
};

function value(item: TicketElement, data: Data) {
  const values: Record<string, string> = {
    EVENT_TITLE: data.eventTitle,
    EVENT_DATE: data.date,
    EVENT_TIME: data.time,
    VENUE: data.venue,
    ADDRESS: data.address,
    CUSTOMER_NAME: data.customerName,
    TICKET_TYPE: data.ticketType,
    ORDER_NUMBER: data.orderNumber,
    TICKET_CODE: data.ticketCode,
    CUSTOM: item.content,
  };
  return values[item.binding] ?? item.content;
}

function elementClass(item: TicketElement) {
  return `customer-ticket-element customer-ticket-element--${item.binding.toLowerCase().replaceAll("_", "-")}`;
}

function responsiveFontSize(item: TicketElement) {
  const minimum = Math.min(item.binding === "EVENT_TITLE" ? 18 : 10, item.fontSize);
  return `clamp(${minimum}px, ${(item.fontSize / 360) * 100}vw, ${item.fontSize}px)`;
}

export function TicketCard({
  ticket,
  qr,
  design,
  event,
  orderNumber,
  walletReady,
}: {
  ticket: { id: string; publicCode: string; status: string; holderName: string; category: { name: string } };
  qr: string;
  design: TicketDesign;
  event: { title: string; startsAt: Date; venue: { name: string; address: string } };
  orderNumber: string;
  walletReady: boolean;
}) {
  const locale = getTicketLocale(design as never);
  const date = formatTicketDate(event.startsAt, locale);
  const time = formatTicketTime(event.startsAt, locale);
  const data = {
    eventTitle: event.title,
    date,
    time,
    venue: event.venue.name,
    address: event.venue.address,
    customerName: ticket.holderName,
    ticketType: ticket.category.name,
    orderNumber,
    ticketCode: ticket.publicCode,
  };
  const darkLogo = design.backgroundColor.toUpperCase() === "#081426";

  return (
    <article className="customer-ticket-wrap">
      <div
        dir={locale === "he" ? "rtl" : "ltr"}
        className="customer-ticket"
        style={{
          backgroundColor: design.backgroundColor,
          backgroundImage: design.backgroundUrl
            ? `linear-gradient(#08142677,#081426bb),url(${design.backgroundUrl})`
            : undefined,
        }}
      >
        {design.logoUrl ? (
          <img className="customer-ticket-logo" src={design.logoUrl} alt="Логотип организатора" />
        ) : (
          <div
            className="customer-ticket-atlas-logo"
            style={{ left: locale === "he" ? undefined : 27, right: locale === "he" ? 27 : undefined }}
          >
            <AtlasLogo href="/" dark={darkLogo} />
          </div>
        )}

        {design.elements
          .filter((item) => !item.hidden)
          .map((item) => {
            const isEventTitle = item.binding === "EVENT_TITLE";
            const titleStyle = isEventTitle
              ? {
                  left: "8%",
                  top: "27%",
                  width: "84%",
                  height: "16%",
                  fontSize: "clamp(20px, 6.3vw, 30px)",
                  textAlign: locale === "he" ? ("right" as const) : ("left" as const),
                  fontWeight: 800,
                }
              : {
                  left: `${item.x}%`,
                  top: `${item.y}%`,
                  width: `${item.width}%`,
                  height: `${item.height}%`,
                  fontSize: responsiveFontSize(item),
                  textAlign: item.align,
                  fontWeight: item.bold ? 800 : 400,
                };

            return (
              <div
                key={item.id}
                className={elementClass(item)}
                style={{
                  ...titleStyle,
                  color: item.color,
                }}
              >
                {item.binding === "QR" ? (
                  <Image src={qr} alt="Ticket QR code" width={260} height={260} unoptimized />
                ) : item.binding === "IMAGE" ? (
                  <img src={item.content} alt="Оформление билета" />
                ) : (
                  <span>{value(item, data)}</span>
                )}
              </div>
            );
          })}
      </div>

      <div className="ticket-delivery-actions">
        <Link className="btn secondary" href={`/api/tickets/${ticket.id}/pdf`}>
          Скачать PDF
        </Link>
        {walletReady ? (
          <Link className="apple-wallet-button" href={`/api/wallet/tickets/${ticket.id}`}>
            <WalletCards size={20} />
            <span>
              <small>Добавить в</small>
              Apple Wallet
            </span>
          </Link>
        ) : (
          <span className="wallet-unavailable" title="Добавьте Apple Wallet сертификаты в окружение">
            <WalletCards size={18} />
            Apple Wallet будет доступен после подключения сертификата
          </span>
        )}
      </div>
    </article>
  );
}
