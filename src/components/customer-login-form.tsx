...
export function CustomerLoginForm({ expired = false }: { expired?: boolean }) {
  ...
  async function submit(event: React.FormEvent<HTMLFormElement>) {
    ...
    if (!response.ok) {
      setError(true);
      setMessage(data.error || text.error);
      return;
    }
  ...
  return <>
    <header className={styles.brand}>
      ...
      <p className={styles.subtitle}>{text.help}</p>
    </header>

    {message && <div className={styles.notice} style={error ? { background: "#fff1f0", color: "#b42318" } : { background: "#ecfdf3", color: "#166534" }}>{message}</div>}
    ...
    <form className={styles.form} onSubmit={submit}>
      ...
      <button className={styles.primaryButton} disabled={busy}>{busy ? text.sending : text.send}</button>
    </form>
