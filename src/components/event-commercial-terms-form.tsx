...
export function EventCommercialTermsForm({ eventId, initial, organizerName, isSuperAdmin }: {
  ...
  }), [effectivePayer, initial.salesFeeFixedMinor, initial.salesFeePercentBps]);
  const percentLabel = `${(initial.salesFeePercentBps / 100).toLocaleString("ru-RU", { maximumFractionDigits: 2 })}%`;
  const feeLabel = `${percentLabel}${initial.salesFeeFixedMinor ? ` + ${money(initial.salesFeeFixedMinor)}` : ""}`;
  ...
  return <section className="card" style={{ marginTop: 20, padding: 26, overflow: "visible" }}>
    ...
    {isSuperAdmin && <div style={{ marginTop: 18, padding: 14, borderRadius: 14, background: "#eef5ff", border: "1px solid #cfe0ff" }}>
      <strong>Организатор: {organizerName}</strong>
    </div>}
    ...
    <label style={{ marginTop: 18, padding: 16, border: "1px solid var(--line)", borderRadius: 14, display: "flex", alignItems: "flex-start", gap: 12, cursor: "pointer" }}>
      ...
      <span style={{ display: "grid", gap: 4 }}>
        <strong>Использовать стандартное правило организатора</strong>
        <small className="muted">Сейчас для {organizerName}: {initial.organizerServiceFeePayer === "BUYER" ? "сервисный сбор оплачивает покупатель" : "сервисный сбор оплачивает организатор"}.</small>
      </span>
    </label>

    {!useOrganizerDefaults && <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(280px,1fr))", gap: 14, marginTop: 16 }}>
      <button type="button" onClick={() => setServiceFeePayer("BUYER")} style={optionStyle(serviceFeePayer === "BUYER")}>
        <span style={{ fontSize: 22 }}>{serviceFeePayer === "BUYER" ? "✓ " : ""}<strong>Платит покупатель</strong></span>
        <span>Сбор добавляется сверху к стоимости билетов.</span>
      </button>
      <button type="button" onClick={() => setServiceFeePayer("ORGANIZER")} style={optionStyle(serviceFeePayer === "ORGANIZER")}>
        <span style={{ fontSize: 22 }}>{serviceFeePayer === "ORGANIZER" ? "✓ " : ""}<strong>Плачу я как организатор</strong></span>
        <span>Покупатель платит только за билеты, а сбор удерживается из вашей выплаты.</span>
      </button>
    </div>}
