...
export function CategoryManager({eventId,categories}:{eventId:string;categories:ManagedCategory[]}){
 const{locale}=useLocale();const text=copy[locale];const[editingId,setEditingId]=useState<string|null>(null);const[message,setMessage]=useState("");const[error,setError]=useState("");const[busy,setBusy]=useState(false);const[deletingId,setDeletingId]=useState<string|null>(null);
 ...
 async function remove(item:ManagedCategory){
  if(item.sold>0){setError(text.deleteBlocked);return;}
  if(!window.confirm(`${text.deleteConfirm}\n\n${item.name}`))return;
  setMessage("");setError("");setDeletingId(item.id);
  try{
   const response=await fetch(`/api/admin/events/${encodeURIComponent(eventId)}/categories/${encodeURIComponent(item.id)}`,{method:"DELETE"});
   const raw=await response.text();let data:{error?:string}={};try{data=raw?JSON.parse(raw):{};}catch{}
   if(!response.ok){setError(data.error||text.deleteError);return;}
   window.location.reload();
  }catch{setError(text.deleteError);}finally{setDeletingId(null);}
 }
 return <div className="table-wrap"><table><thead><tr><th>{text.category}</th><th>{text.priceNow}</th><th>{text.sold}</th><th>{text.remaining}</th><th/></tr></thead><tbody>{categories.map(item=><Fragment key={item.id}><tr style={item.hidden?{opacity:.55}:undefined}><td><strong>{item.name}</strong>{item.hidden&&<div className="muted" style={{fontSize:12,marginTop:2}}>{text.hidden}</div>}</td><td>{item.currentPriceMinor!==null?money(item.currentPriceMinor):<span className="muted">{item.statusLabel}</span>}{item.pricingMode==="SCHEDULED"&&<div className="pill" style={{marginTop:6}}>{text.bySchedule}</div>}{item.nextTierPriceMinor!==undefined&&item.nextTierStartsAt&&<div className="muted" style={{fontSize:12,marginTop:4}}>{money(item.nextTierPriceMinor)} · {new Date(item.nextTierStartsAt).toLocaleDateString(locale==="he"?"he-IL":locale==="en"?"en-IL":"ru-RU")}</div>}</td><td>{item.sold}</td><td>{item.capacity-item.sold}</td><td><div className="row" style={{flexWrap:"wrap"}}><button type="button" className="btn secondary" onClick={()=>{setError("");setEditingId(editingId===item.id?null:item.id);}}>{editingId===item.id?text.cancel:text.edit}</button><button type="button" className="btn secondary" disabled={busy||deletingId!==null} onClick={()=>void send({action:"category-visibility",categoryId:item.id,hidden:!item.hidden})}>{item.hidden?text.show:text.hide}</button><button type="button" className="btn secondary" disabled={busy||deletingId!==null||item.sold>0} title={item.sold>0?text.deleteBlocked:undefined} onClick={()=>void remove(item)} style={item.sold===0?{borderColor:"#dc2626",color:"#dc2626"}:undefined}>{deletingId===item.id?text.deleting:text.delete}</button></div></td></tr>{editingId===item.id&&<tr><td colSpan={5}><CategoryEditForm category={item} busy={busy} error={error} onSave={body=>send({action:"category-update",categoryId:item.id,...body})}/></td></tr>}</Fragment>)}</tbody></table>{message&&<div className="toast">{message}</div>}{error&&!editingId&&<div className="toast" role="alert">{error}</div>}</div>;
}
