...
type SavedCategory={id:string;name:string;description:string|null|string;priceMinor:number;pricingMode:"FIXED"|"SCHEDULED";capacity:number;sold:number;hidden:boolean;colorHex:string;maxPerOrder:number;salesStart:string|null;salesEnd:string|null;priceTiers:Tier[]};

function dateInput(iso:string|null|undefined){return iso?new Date(iso).toISOString().slice(0,10):"";}
function optionalStartOfDayIso(value:FormDataEntryValue|null){
 const date=String(value||"").trim();
 if(!date)return undefined;
 if(!/^\d{4}-\d{2}-\d{2}$/.test(date))throw new Error("INVALID_DATE");
 return new Date(`${date}T00:00:00.000`).toISOString();
}
function optionalEndOfDayIso(value:FormDataEntryValue|null){
 const date=String(value||"").trim();
 if(!date)return undefined;
 if(!/^\d{4}-\d{2}-\d{2}$/.test(date))throw new Error("INVALID_DATE");
...
function CategoryEditForm({category,onSave,busy,error}:{category:ManagedCategory;onSave:(body:Record<string,unknown>)=>Promise<void>;busy:boolean;error:string}){
 ...
 return <form className="pricing-stage-editor" onSubmit={async event=>{event.preventDefault();const form=new FormData(event.currentTarget);try{const salesStart=optionalStartOfDayIso(form.get("salesStart"));const salesEnd=optionalEndOfDayIso(form.get("salesEnd"));const changeAt=pricingMode==="SCHEDULED"?nextDayStartIso(form.get("changeDate")):undefined;await onSave({name:String(form.get("name")||"").trim(),description:String(form.get("description")||"").trim(),colorHex:String(form.get("colorHex")||""),priceMinor:Math.round(Number(form.get(pricingMode==="SCHEDULED"?"nextPrice":"currentPrice"))*100),capacity:Number(form.get("capacity")),pricingMode,salesStart,salesEnd,earlyBirdPriceMinor:pricingMode==="SCHEDULED"?Math.round(Number(form.get("currentPrice"))*100):undefined,earlyBirdEndsAt:changeAt,maxPerOrder:Number(form.get("maxPerOrder"))});}catch{await onSave({__clientError:text.saveError});}}}>
...
export function CategoryManager({eventId,categories}:{eventId:string;categories:ManagedCategory[]}){
 const router=useRouter();const{locale}=useLocale();const text=copy[locale];const[items,setItems]=useState(categories);const[editingId,setEditingId]=useState<string|null>(null);const[message,setMessage]=useState("");const[error,setError]=useState("");const[busy,setBusy]=useState(false);
 useEffect(()=>setItems(categories),[categories]);
 async function send(body:Record<string,unknown>){
  if(body.__clientError){setError(String(body.__clientError));return;}
  setMessage("");setError("");setBusy(true);
  try{
   const isUpdate=body.action==="category-update";const categoryId=String(body.categoryId||"");const payload=isUpdate?Object.fromEntries(Object.entries(body).filter(([key])=>key!=="action"&&key!=="categoryId")):body;
   const endpoint=isUpdate?`/api/admin/events/${eventId}/category/${encodeURIComponent(categoryId)}`:`/api/admin/events/${eventId}`;
   const response=await fetch(endpoint,{method:"PATCH",headers:{"content-type":"application/json","cache-control":"no-store"},cache:"no-store",body:JSON.stringify(payload)});
   const raw=await response.text();let data:{error?:string;category?:SavedCategory}={};try{data=raw?JSON.parse(raw):{};}catch{}
   if(!response.ok){setError(data.error||text.saveError);return;}
   if(isUpdate){
    if(!data.category||data.category.id!==categoryId||data.category.name!==String(payload.name||"").trim()){setError(text.saveError);return;}
    const saved=data.category;
    setItems(current=>current.map(item=>item.id===saved.id?{...item,...saved,description:saved.description||null,currentPriceMinor:saved.pricingMode==="FIXED"?saved.priceMinor:item.currentPriceMinor}:item));
   }else if(body.action==="category-visibility"){
    setItems(current=>current.map(item=>item.id===String(body.categoryId)?{...item,hidden:Boolean(body.hidden)}:item));
   }
   setMessage(text.saved);setEditingId(null);router.refresh();
  }catch{setError(text.saveError);}finally{setBusy(false);}
 }
 return <div className="table-wrap"><table><thead><tr><th>{text.category}</th><th>{text.priceNow}</th><th>{text.sold}</th><th>{text.remaining}</th><th/></tr></thead><tbody>{items.map(item=><Fragment key={item.id}><tr style={item.hidden?{opacity:.55}:undefined}><td><strong>{item.name}</strong>{item.hidden&&<div className="muted" style={{fontSize:12,marginTop:2}}>{text.hidden}</div>}</td><td>{item.currentPriceMinor!==null?money(item.currentPriceMinor):<span className="muted">{item.statusLabel}</span>}{item.pricingMode==="SCHEDULED"&&<div className="pill" style={{marginTop:6}}>{text.bySchedule}</div>}{item.nextTierPriceMinor!==undefined&&item.nextTierStartsAt&&<div className="muted" style={{fontSize:12,marginTop:4}}>{money(item.nextTierPriceMinor)} · {new Date(item.nextTierStartsAt).toLocaleDateString(locale==="he"?"he-IL":locale==="en"?"en-IL":"ru-RU")}</div>}</td><td>{item.sold}</td><td>{item.capacity-item.sold}</td><td><div className="row"><button type="button" className="btn secondary" onClick={()=>{setError("");setEditingId(editingId===item.id?null:item.id);}}>{editingId===item.id?text.cancel:text.edit}</button><button type="button" className="btn secondary" disabled={busy} onClick={()=>void send({action:"category-visibility",categoryId:item.id,hidden:!item.hidden})}>{item.hidden?text.show:text.hide}</button></div></td></tr>{editingId===item.id&&<tr><td colSpan={5}><CategoryEditForm category={item} busy={busy} error={error} onSave={body=>send({action:"category-update",categoryId:item.id,...body})}/></td></tr>}</Fragment>)}</tbody></table>{message&&<div className="toast">{message}</div>}</div>;
}
