...
let studioElements=[{id:"title",text:"NOA ELECTRIC — LIVE",x:8,y:9,size:30,bold:true},{id:"type",text:"VIP SEATING",x:8,y:23,size:14,bold:true},{id:"date",text:"18 сентября 2026 · 21:30",x:8,y:33,size:16,bold:true},{id:"venue",text:"Hangar 11, Tel Aviv",x:8,y:42,size:14},{id:"guest",text:"Igor Levin",x:8,y:53,size:17,bold:true},{id:"qr",text:"▦",x:32,y:66,size:92,bold:true},{id:"code",text:"ATL-TKT-8F2K-92MA",x:25,y:91,size:10}];
...
function ticketStudio(){app.innerHTML=`<div class="studio-demo-page"><a class="back" href="#admin">← Настройки мероприятия</a><div class="studio-demo-head"><div><span class="eyebrow">Ticket studio</span><h1>Редактор билета</h1><p>Текст, фотографии, системные поля, QR, PDF и Apple Wallet.</p></div><button id="studio-save" class="button">Сохранить шаблон</button></div><div class="studio-demo"><aside><h3>Добавить</h3><button data-add-studio="text">T <span>Произвольный текст</span></button><button data-add-studio="image">▧ <span>Фотография</span></button><button data-add-studio="date">◷ <span>Дата и время</span></button><button data-add-studio="venue">⌖ <span>Площадка</span></button><button data-add-studio="guest">♙ <span>Имя гостя</span></button><button data-add-studio="qr">▦ <span>QR-код</span></button><h3>Цвет билета</h3><input id="studio-color" type="color" value="#081426"></aside><main><div class="studio-phone"><div id="studio-canvas" class="studio-ticket">${studioElements.map(item=>`<button data-studio-element="${item.id}" style="left:${item.x}%;top:${item.y}%;font-size:${item.size}px;font-weight:${item.bold?800:400}">${item.text}</button>`).join("")}</div></div><small>Перетащите любой элемент в нужное место</small></main><aside class="studio-props"><h3>Свойства элемента</h3><p>Выберите элемент на билете.</p><label>Размер<input id="studio-size" type="range" min="9" max="54" value="18"></label><div><button>Жирный</button><button>По центру</button></div><hr><h3>Apple Wallet</h3><p>Дата и площадка связаны с мероприятием. После изменения Atlas отправит обновление в установленный билет.</p><span class="wallet-demo-badge"> Add to Apple Wallet</span></aside></div></div>`;let selected=null;document.querySelectorAll("[data-studio-element]").forEach(element=>{element.onpointerdown=e=>{selected=studioElements.find(item=>item.id===element.dataset.studioElement);document.querySelectorAll("[data-studio-element]").forEach(x=>x.classList.remove("selected"));element.classList.add("selected");element.setPointerCapture(e.pointerId)};element.onpointermove=e=>{if(!selected||!element.hasPointerCapture(e.pointerId))return;const rect=document.querySelector("#studio-canvas").getBoundingClientRect();selected.x=Math.max(2,Math.min(90,(e.clientX-rect.left)/rect.width*100));selected.y=Math.max(2,Math.min(94,(e.clientY-rect.top)/rect.height*100));element.style.left=`${selected.x}%`;element.style.top=`${selected.y}%`};element.onpointerup=e=>{if(element.hasPointerCapture(e.pointerId))element.releasePointerCapture(e.pointerId)}});document.querySelector("#studio-color").oninput=e=>document.querySelector("#studio-canvas").style.background=e.target.value;document.querySelector("#studio-size").oninput=e=>{if(selected){selected.size=Number(e.target.value);document.querySelector(`[data-studio-element="${selected.id}"]`).style.fontSize=`${selected.size}px`}};document.querySelector("#studio-save").onclick=e=>e.target.textContent="Шаблон сохранён ✓";document.querySelectorAll("[data-add-studio]").forEach(button=>button.onclick=()=>{studioElements.push({id:`new-${Date.now()}`,text:button.dataset.addStudio==="image"?"▧ ФОТО":"Новый текст",x:12,y:58,size:18,bold:false});ticketStudio()})}
