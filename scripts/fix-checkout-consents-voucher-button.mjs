...
if(!src.includes('ATLAS_CONSENTS_VOUCHER_BUTTON')){
  ...
  const consentBlock=`<div className={styles.consentList}>
   ...
   <label className={\`${'${styles.consentRow}'}${'${consentOrganizerInvalid?` ${styles.consentInvalid}`:""}'}\`}><input type="checkbox" checked={consentOrganizer} onChange={e=>setConsentOrganizer(e.target.checked)}/><span>{locale==="he"?"אני מאשר/ת קבלת מידע וחומר פרסומי ממארגן/ת האירוע, מסכים/ה לתנאי מועדון הלקוחות שלו/ה, ואם איני חבר/ה עדיין - מאשר/ת את רישומי למועדון באמצעות מערכת הנאמנות המחוברת של המארגן/ת.":locale==="ru"?"Я согласен(на) получать информацию и рекламные материалы от организатора мероприятия, принимаю условия его клубной программы и, если я ещё не являюсь участником, разрешаю зарегистрировать меня в клубе через подключённую организатором систему лояльности.":"I agree to receive information and promotional materials from the event organizer, accept the organizer's club terms, and, if I am not yet a member, authorize my registration in the organizer's club through the organizer's connected loyalty system."}</span></label>
  </div>`;
