...
// Voucher validation must happen before an order exists. Use the checkout
// preview endpoint instead of the old order-scoped promo endpoint.
src=src.replace(
...
// Persist the remaining checkout details before leaving ATLAS, then navigate
// the top-level browsing context to the HYP launch URL returned by the API.
...
// Keep the voucher UI on ATLAS before the payment CTA.
...
// CTA validates everything first, then starts checkout. Do not switch to an
// iframe/payment stage - the next visible page is HYP itself.
src=src.replaceAll('setPaymentStage(true);void startPayment();','void startPayment();');

// Never render the legacy embedded payment area.
